		...
		case SELECT_ORGS:
			...
			synchronized(BroadcastQueue.loader_monitor){
				...
				if(crt_orgs_idx >= crt_orgs_queue.size()){
					...
					if((count_broadcasted_orgs >= broadcasts)&& (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadO = true;
					}
				}
			}
		...
		case SELECT_CONS:
			...
			synchronized(BroadcastQueue.loader_monitor){
				...
				if(crt_constituents_idx >= crt_constituents_queue.size()){
					...
					if((count_broadcasted_constituents >= broadcasts) && (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadC = true;
					}
				}
			}
		...
		case SELECT_VOTE:
			...
			synchronized(BroadcastQueue.loader_monitor){
				...
				if(crt_votes_idx >= crt_votes_queue.size()){
					...
					if((count_broadcasted_votes >= broadcasts)&& (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadV = true;
					}
				}
			}
		...
		case SELECT_WITN:
			...
			synchronized(BroadcastQueue.loader_monitor){
				...
				if(crt_witnesses_idx >= crt_witnesses_queue.size()){
					...
					if((count_broadcasted_witnesses >= broadcasts) && (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadW = true;
					}
				}
			}
		...
		case SELECT_PEER:
			...
			synchronized(BroadcastQueue.loader_monitor){
				...
				if(crt_peers_idx >= crt_peers_queue.size()){
					...
					if((count_broadcasted_peers >= broadcasts) && (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadP = true;
					}
				}
			}
		...
		case SELECT_NEIG:
			...
			synchronized(BroadcastQueue.loader_monitor ) {
				...
				if(crt_neighbors_idx >= crt_neighbors_queue.size()) {
					...
					if((count_broadcasted_neighborhoods >= broadcasts) && (!simulator.WirelessLog.Handled_queue.equals(queue_type))) {
						toLoadN = true;
					}
				}
			}
