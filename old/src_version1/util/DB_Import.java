...
public class DB_Import {
	...
	public static boolean import_db(String previous, String db_new) throws P2PDDSQLException {
		...
		if(error_db != null) {
			...
			Application.warning(_("Fail to open old database:")+"\n\""+db_to_import+"\"\n"+error_db, _("Abandon database import!"));
			return false;
		}
		...
		case 1: //no
			...
			chooser.setName(_("Select database to import from"));
			chooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
			chooser.setMultiSelectionEnabled(false);
			File fileDB = null;

			int returnVal = chooser.showDialog(DD.frame,_("Specify Database"));
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					if (error == null){
						db_to_import = fileDB.getAbsolutePath();
						break;
					}
					Application.warning(_("Failure to open:")+fileDB+"\nError: "+error, _("Failure to open!"));
				} catch (IOException e) {
					e.printStackTrace();
					Application.warning(_("Failure to open:")+fileDB+"\nException: "+e.getLocalizedMessage(), _("Failure to open!"));
				}
			}
			int c = Application.ask(_("Do you want to exit?")+"\n"+
		...
		try {
			...
			e.printStackTrace();
			Application.warning(_("Error importing: "+e.getLocalizedMessage()), _("Error importing"));
		}
