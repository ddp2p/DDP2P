...
public class D_Message extends ASNObj {
	...
	public Encoder getEncoder() {
		...
		if(sender!=null) enc.addToSequence(sender.getEncoder().setASN1Type(DD.TAG_AC0));
		if(Peer!=null) enc.addToSequence(Peer.getEncoder().setASN1Type(DD.TAG_AC1));
		if(interest!=null) enc.addToSequence(interest.getEncoder().setASN1Type(DD.TAG_AC2));
		if(organization!=null)enc.addToSequence(organization.getEncoder().setASN1Type(DD.TAG_AC3));
		if(motion!=null) enc.addToSequence(motion.getEncoder().setASN1Type(DD.TAG_AC4));
		if(constituent!=null) enc.addToSequence(constituent.getEncoder().setASN1Type(DD.TAG_AC5));
		if(witness!=null) enc.addToSequence(witness.getEncoder().setASN1Type(DD.TAG_AC6));
		if(vote!=null)enc.addToSequence(vote.getEncoder().setASN1Type(DD.TAG_AC7));
		if(signature!=null) enc.addToSequence(new Encoder(signature).setASN1Type(DD.TAG_AC8));
		if(neighborhoods!=null) enc.addToSequence(Encoder.getEncoder(neighborhoods).setASN1Type(DD.TAG_AC9));
		if(recent_senders!=null){
	...
	public D_Message decode(Decoder decoder) throws ASN1DecoderFail {
		...
		if(dec.getTypeByte()==DD.TAG_AC0)sender = new D_PeerAddress().decode(dec.getFirstObject(true));
		if(dec.getTypeByte()==DD.TAG_AC1)Peer = new D_PeerAddress().decode(dec.getFirstObject(true));
		if(dec.getTypeByte()==DD.TAG_AC2) interest = new WB_ASN_Interest().decode(dec.getFirstObject(true));
		if(dec.getTypeByte()==DD.TAG_AC3)organization = new D_Organization().decode(dec.getFirstObject(true));
		if(dec.getTypeByte()==DD.TAG_AC4)motion = new D_Motion().decode(dec.getFirstObject(true));
		if(dec.getTypeByte()==DD.TAG_AC5)constituent = new D_Constituent().decode(dec.getFirstObject(true));
		...
		if(dec.getTypeByte()==DD.TAG_AC9)neighborhoods = dec.getFirstObject(true).getSequenceOf(Encoder.TYPE_SEQUENCE, new D_Neighborhood[]{}, new D_Neighborhood());
