...
public class UpdatePeersTable {
	...
	/**
	 * returns the peers with GID given by "filteredID"
	 * 
	 * @param asa
	 * @param peer
	 * @param tab
	...
	static Table buildPeersTable2(String last_sync_date, String[] _maxDate, boolean justDate, HashSet<String> orgs2, int limitPeersLow, int limitPeersMax) throws P2PDDSQLException{
		if(justDate){
			...
			if(_DEBUG) System.out.println("UpdatePeersTable:buildPeersTable2: result justDate maxDate="+_maxDate[0]);
			return null;
		}
		//boolean DEBUG = true;
	...
	public static D_Peer integratePeersTable2(ASNSyncPayload asa, D_Peer peer, Table tab, String filteredID) throws P2PDDSQLException{
		...
		if ( D_Peer.getPeersCount() > DD.ACCEPT_STREAMING_UPTO_MAX_PEERS) {
			Application_GUI.warning(Util._("Upper limit for number of peers is reached:")+DD.ACCEPT_STREAMING_UPTO_MAX_PEERS+
		...
		for (int i = 0; i < tab.rows.length; i ++) {
			...
			*/
			if (tab.rows[i].length <= 9) {
				if (DEBUG || DD.DEBUG_TODO) Application_GUI.warning(Util._("Peer has wrong message format"), Util._("Old version peer"));
			 ...
			 */
			D_Peer received = new D_Peer();
			received.setGID(global_peer_ID, null);
			received.served_orgs = table.peer_org.peerOrgsFromString(global_organizationIDs);
			...
			received.component_basic_data.creation_date = Util.getCalendar(creation_date);
			...
			// Verify signature of peer
			D_Peer peer_data = D_Peer.getPeerByGID(global_peer_ID, true, true);
			peer_data = D_Peer.getPeerByPeer_Keep(peer_data);
			if (peer_data._peer_ID <= 0) {
				peer_data.setUsed(_used);//.component_preferences.used = _used;
				peer_data.setProvider(peer.peer_ID);// component_local_agent_state.first_provider_peer = peer.peer_ID;
				
				peer_data.setBlocked(//.component_preferences.blocked =
