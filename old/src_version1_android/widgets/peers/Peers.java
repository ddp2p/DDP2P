...
public class Peers extends JTable implements Peers_View, MouseListener, PeerListener {
	...
	//area between 4x-x^2   and x-2
   	public void connectWidget(){
		...
		Application.peers = (config.Peers_View) this;
	...
	void init(){
		...
		icon_register = widgets.app.DDIcons.getRegistrationImageIcon("Register");//Util.createImageIcon("icons/sad.smiley10.gif","General Org");
		icon_org = widgets.app.DDIcons.getOrgImageIcon("Org");//Util.createImageIcon("icons/sad.smiley10.gif","General Org");
...
//        			_("New Myself!"),KeyEvent.VK_N, Peers.COMMAND_MENU_NEW_MYSELF));
       	use_stop = new JMenuItem(new PeersUseAction(_this, _("Do not access it!"),addicon,_("Stop pulling from it."),_("Will not be used to synchronize."),KeyEvent.VK_A));
       	use_start = new JMenuItem(new PeersUseAction(_this, _("Access it!"),addicon,_("Pull from it."),_("Will be used to synchronize."),KeyEvent.VK_A));
    	//
	...
	class DispatchPeer extends util.DDP2P_ServiceThread {
		Peers peers;
		int model_row;
		private boolean me;
		...
		public void _run() {
			...
			if (peer == null) {
				...
				if (model_row >= 0){
					...
					if (id <= 0) return;
					peer = D_Peer.getPeerByLID(id, true);
...
class PeersResetAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	D_Peer.reset(peerID, null, Util.CalendargetInstance());
    }
}
@SuppressWarnings("serial")
class PeersDeleteAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	String peerID = peer.get_ID(); //Util.getString(model.getPeers().get(row).get(0));
    	int d = Application_GUI.ask(_("Are you willing to delete peer:")+" #"+peerID+":"+peer.getName(),
...
class PeersRowAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
		...
		case Peers.COMMAND_MENU_SHARE_ORG:
			...
			if(org_ID == null){
				Application_GUI.warning(_("No Organization selected"), _("Failure Adding Peer"));
				break;
			}
			try {
		...
		case Peers.COMMAND_TOUCH_CLIENT:
			try {
				...
				e4.printStackTrace();
				Application_GUI.warning(_("Error trying to restart comunication:")+"\n "+e4.getLocalizedMessage(), _("Error trying to restart communication!"));
			}
			break;
		case Peers.COMMAND_MENU_SET_NAME:
			try {
				...
				e4.printStackTrace();
				Application_GUI.warning(_("Error trying to change names:")+"\n "+e4.getLocalizedMessage(), _("Error trying to change names!"));
			}
			break;
		case Peers.COMMAND_MENU_SET_SLOGAN:
			try {
				...
				e4.printStackTrace();
				Application_GUI.warning(_("Error trying to change slogan:")+"\n "+e4.getLocalizedMessage(), _("Error trying to change slogan!"));
			}
			break;
		case Peers.COMMAND_MENU_SET_EMAILS:
			try {
				...
				e4.printStackTrace();
				Application_GUI.warning(_("Error trying to change emails:")+"\n "+e4.getLocalizedMessage(), _("Error trying to change emails!"));
			}
			break;
...
//			}
		...
		case Peers.COMMAND_MENU_SAVE_SK:
			...
			if(fileTrustedSK.exists()) {
				int c = Application_GUI.ask(_("Existing file. Overwrite: "+fileTrustedSK+"?"), _("Overwrite file?"), JOptionPane.OK_CANCEL_OPTION);
				if(c!=0) break;
			}
			...
			try {
				...
				Application_GUI.warning(_("Failed to save key: "+e3.getMessage()), _("Failed to save key"));
				e3.printStackTrace();
				break;
				...
				Application_GUI.warning(_("Failed to save key: "+e4.getMessage()), _("Failed to save key"));
				e4.printStackTrace();
				break;
			}
			Application_GUI.warning(_("Failed to save key: absent"), _("Failed to save key"));
...
//					}
    	...
    	case Peers.COMMAND_MENU_SET_MYSELF:
    		...
    		try {
    			D_Peer me_peer = D_Peer.getPeerByLID(peerID, false);
    			String current = me_peer.getInstance();
    			String expected = HandlingMyself_Peer.peekSomeInstance(me_peer);
    			switch (Application_GUI.ask(_("Do you want to set the Noname instance?\nRather than: expected=\"")+expected+"\" / current=\""+current+"\"", _("Noname instance?"), JOptionPane.YES_NO_CANCEL_OPTION)) {
    			case 0:
...
//					}
				...
				e1.printStackTrace();
				Application_GUI.warning(_("Failure to set new ID:")+"\n"+e1.getLocalizedMessage(), _("Failure in message!"));
			}
    		break;
    	default:
    	}
	...
	public static D_Peer loadPeer(Component parent) {
		...
		//filterUpdates.setSelectedFile(null);
		Util_GUI.cleanFileSelector(filterUpdates);
		int loadNewPeerVal = filterUpdates.showDialog(parent,_("Specify Trusted Secret Key File"));
		...
		try{
			...
			if (!is_new[0]) {
				Application_GUI.warning(_("Secret key already available!"), _("Loading Secret Key"));
				return null;
			}
			...
			peer = D_Peer.getPeerByGID(new_gid, true, true, null);
...
class PeersModel extends AbstractTableModel implements TableModel, DBListener {
	...
	final String columnNames[]={_("Safes' Name"),"V","Serving",
			"Connection","Br","B","H","S","T","R","Email","V","Categ","Slogan","Provider","LastSync","Pluggins"};
	protected final String[] columnToolTips = {
			_("The name declared by the peer (white for default broadcastable)!")
	...
	D_Peer getPeer(int model_row) {
		...
		return D_Peer.getPeerByLID(id, true);
	}
	...
	public Object getAlias(int model_row) {
		return getPeer(model_row).getMyName();
	}
	/**
	 * Get GID for plugins
	 * @param model_row
	...
	public String getGID(int model_row) {
		return getPeer(model_row).getGID();
	}
...
/*		
	...
	util.DDP2P_ServiceThread refresher;
	PeersModel(DBInterface _db) {
		...
		refresher = new util.DDP2P_ServiceThread("Peers Refresher", true, this) {
			public void _run() {
				for (;;) {
...
//		}
	...
	public Object getValueAt(int row, int col) {
		...
		D_Peer peer = D_Peer.getPeerByLID(peerID, true); //getPeer(row);
	...
	*/
	...
	public void setValueAt(Object value, int row, int col) {
		...
		case TABLE_COL_HIDDEN:
			...
			break;
		case TABLE_COL_TEMP:
			if(0 == Application_GUI.ask(_("Do you really want to drop signature for:"+" "+getName(row)),
			...
			break;
		case TABLE_COL_VALID:
			if(0 == Application_GUI.ask(_("Do you really want to attempt re-signature for:"+" "+peer.getName()),
					_("Are you sure?"), JOptionPane.OK_CANCEL_OPTION)){
