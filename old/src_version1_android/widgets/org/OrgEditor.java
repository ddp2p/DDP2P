...
public class OrgEditor  extends JPanel implements OrgListener, ActionListener, FocusListener, DocumentListener, LVListener, ItemListener, MultiformatDocumentListener {
	...
	//	tabbedPane.setLayout(new BorderLayout());
		...
		ImageIcon icon = widgets.app.DDIcons.getOrgImageIcon("General Org"); //Util.createImageIcon("icons/sad.smiley10.gif","General Org");
		...
		ImageIcon icon_register = widgets.app.DDIcons.getRegistrationImageIcon("Register");//Util.createImageIcon("icons/sad.smiley10.gif","General Org");
		ImageIcon icon_preapproved = widgets.app.DDIcons.getConImageIcon("Preapproved Constituents");//Util.createImageIcon("icons/sad.smiley10.gif","General Org");
...
//			}
		...
		if((commit_field==source)) {
			...
			if((selected == null) || (selected.gid == null)){
				Application_GUI.warning(_("No Identity created/selected"), _("Organization not ready"));
				return;
			}
			try{
				String name = Util.getString(org.get(table.organization.ORG_COL_NAME));
				if((name==null) || ("".equals(name.trim()))){
					Application_GUI.warning(_("You need to select a name for you organization"), _("Missing Name"));
					return;						
				}
				if(org.get(table.organization.ORG_COL_CREATOR_ID)==null){
					Application_GUI.warning(_("You may want to select a peer identity as Initiator"), _("Missing Initiator"));
					//return;					
				}
			}catch(Exception e){}
			...
			*/
			...
			try {
				...
				*/
				...
				e.printStackTrace();
				Application_GUI.warning(_("Error saving organization:")+" "+e.getLocalizedMessage()+"\n"+
			...
			if(method==table.organization._AUTHORITARIAN) {
				...
				try {
					...
					if((org[0]==null) || (org[0].params==null) || (org[0].params.creator_global_ID==null)) {
						Application_GUI.warning(_("You need to select a peer identity as")+" "+_("Initiator"), _("Missing Initiator"));
						return;
					}
					orgData = org[0];
					signature = Util.stringSignatureFromByte(_signature);
...
//					gid = Util.stringSignatureFromByte(_gid);
				...
				if(Application_GUI.ask(_("GrassRoot commision is irreversible.")+"\n"+
