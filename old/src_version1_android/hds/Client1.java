...
public
class Client1 extends util.DDP2P_ServiceThread implements IClient {
	...
	synchronized public void __run() {
		...
		for(int p=0;;p++) {
			...
			D_Peer peer = D_Peer.getPeerByGID(global_peer_ID, true, false, null);  // not yet used when saving new served orgs
			...
			if(Application.aus!=null) Application.aus.addSyncRequests(global_peer_ID, instance);
			...
			for(int a=0; a < p_addresses; a++) {
				...
				if ((Application.aus != null) && ! Application.aus.hasSyncRequests(global_peer_ID, instance)) {
					DD.ed.fireClientUpdate(new CommEvent(this, peer_name, null, "LOCAL", "Stop sending: Received ping confirmation already handled from peer"));
