...
class ServerThread extends util.DDP2P_ServiceThread {
...
//				}
				...
				*/
				...
				if(asr.address!=null){
					asr.address.peer_ID = peer_ID = D_Peer.getLocalPeerIDforGID(asr.address.component_basic_data.globalID);
				}else{
					Application_GUI.warning(_("Peer does not authenticate itself"), _("Contact from unknown peer"));
...
class ThreadAskPull extends util.DDP2P_ServiceThread {
	D_Peer pa;
	String peer_ID;
	ASNSyncRequest asr;
	SocketAddress sa;
	...
	public void _run() {
		...
		try {
			__run();
			Server.queried.remove(pa.getGIDH());
	...
	void save(D_Peer pa) {
		D_Peer local;
		local = D_Peer.getPeerByGID_or_GIDhash (
				pa.component_basic_data.globalID,
				pa.component_basic_data.globalIDhash, 
				true, true);
		local = D_Peer.getPeerByPeer_Keep(local);
		local.loadRemote(pa);
		if (asr.dpi != null) local.integratePeerInstance(asr.dpi);
		local.component_preferences.blocked = pa.component_preferences.blocked;
		local.component_preferences.used = pa.component_preferences.used;
...
public class Server extends util.DDP2P_ServiceThread {
	...
	public static boolean extractDataSyncRequest(ASNSyncRequest asr, SocketAddress sa, Object caller) throws P2PDDSQLException {
		...
		D_Peer pa = asr.address;
		if (pa != null) {
			String peer_ID = null;
			DD.ed.fireServerUpdate(new CommEvent(caller, pa.component_basic_data.name, sa, "Client", "Peer Name decoded"));
			if ((pa != null) && (pa.getSignature() != null) &&
					((pa.component_basic_data.globalID != null) || (pa.component_basic_data.globalIDhash != null))) {
				
				if (queried.contains(pa.getGIDH())) return false; // freed after saving/discarding pa

				boolean verif = pa.verifySignature();
				if (verif) {
					// Here the address only needed at version 2
					D_Peer local = D_Peer.getPeerByGID_or_GIDhash (
							pa.component_basic_data.globalID,
							pa.component_basic_data.globalIDhash, 
							true, false);
					//local = D_Peer.getPeerByPeer_Keep(local);
					if ((local == null)) { // || (local.peer_ID == null)) {
						if (DD.ASK_USAGE_NEW_ARRIVING_PEERS_CONTACTING_ME) {
							queried.add(pa.getGIDH()); // removed in thread
			...
			if((peer_ID != null) || (DD.ACCEPT_STREAMING_SYNC_REQUEST_PAYLOAD_DATA_FROM_UNKNOWN_PEERS)) {
				if(asr.pushChanges != null) {
					...
					String address_ID = D_Peer.getAddressID(isa, peer_ID);
					try {
						D_PeerInstance pi = asr.pushChanges.peer_instance;
						if (pi == null) {pi = asr.dpi;}
						if (pi == null) {pi = new D_PeerInstance(); pi.peer_instance = pa.instance;}
						UpdateMessages.integrateUpdate(asr.pushChanges, isa, caller, pa.component_basic_data.globalID, pi, peer_ID, address_ID, _rq, asr.address);
					} catch (ASN1DecoderFail e) {
						e.printStackTrace();
					}
				}
	...
	public static void detectDomain(int port) throws SocketException{
		...
		Thread t = new DDP2P_ServiceThread("Domain detection: "+Util.getGeneralizedTime(), true) {
			public void _run() {
