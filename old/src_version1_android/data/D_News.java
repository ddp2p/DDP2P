...
class D_News extends ASNObj{
	...
	void init(ArrayList<Object> o) throws P2PDDSQLException {
		...
		global_constituent_ID = Util.getString(o.get(table.news.N_FIELDS+0));
		//global_enhanced_newsID = Util.getString(o.get(table.news.M_FIELDS+1));
		global_organization_ID = Util.getString(o.get(table.news.N_FIELDS+1));
		global_motion_ID = Util.getString(o.get(table.news.N_FIELDS+2));
	...
	public D_News decode(Decoder decoder) throws ASN1DecoderFail {
		...
		if(dec.getTypeByte()==DD.TAG_AC13)motion = new D_Motion().decode(dec.getFirstObject(true));	
		return this;
	}
	public byte[] sign() {
	...
	public byte[] sign(String signer_GID) {
		...
		if(sk==null) {
			...
			Application_GUI.warning(Util._("No secret key! You should select a constituent"), Util._("No secret key!"));
			return null;
		}
