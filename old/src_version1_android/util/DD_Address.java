...
//	public static String TYPE = "\n<<TYPE>>\n";
	...
	*/
	...
	void init_V2(D_Peer dd) {
		...
		creation_date = Encoder.getGeneralizedTime(dd.component_basic_data.creation_date);
		picture = dd.component_basic_data.picture;
		Util.printCallPath("Need to encode addresses");
		/*
		if(encode_addresses && (dd.address!=null)){
			address = "";
			for(int k=0; k<dd.address.length; k++) {
	...
	*/
	...
	public void save() throws P2PDDSQLException {
		...
		if (address == null) {
			...
			Application_GUI.warning(_("No address to save!"), _("Saving failed!"));
			return;
		}
		...
		if (!D_Peer.verify(this)) {
			...
			int c = Application_GUI.ask(_("Wrong signature for:\n"+this.getNiceDescription()+"\nDo you still want it?"), _("Wrong signature"), 
					Application_GUI.OK_CANCEL_OPTION);
			if (c > 0) {
				Application_GUI.warning(_("Saving cancelled!"), _("Cancelled!"));
				return;
			} else Application_GUI.warning(_("Address Saved Anyhow!"), _("Saved Anyhow!"));
		}
		...
		D_Peer.loadPeer(this);
		if (DEBUG) Application_GUI.warning(_("Address Saved as #")+peer_ID+"! ", _("Saved!"));
	}
		/*
		D_Peer pa;
		if (V2.equals(version))
...
//			}
	...
	public byte[] _getBytes() {
		...
		if(V3.equals(version)) return peer.encode();
		throw new RuntimeException("Unknown DDAddress version:"+version);
	}
	...
	public void setBytes(byte[] msg) throws ASN1DecoderFail {
		...
		dec=dec.getContent();
		version = null;
		if (dec.getTypeByte() == DD.TAG_AC0) version = dec.getFirstObject(true).getString();
		
		if (V0.equals(version)) {setBytes_V2(dec); return;}
		if (V1.equals(version)) {setBytes_V2(dec); return;}
		if (V2.equals(version)) {setBytes_V2(dec); return;}
		if (V3.equals(version)) {setBytes_V3(dec); return;}
	}
	public void setBytes_V3(Decoder dec) throws ASN1DecoderFail {
		peer = new D_Peer().decode(dec.getFirstObject(true));
		...
		for (D_PeerInstance i : dpi) {
			peer.putPeerInstance(i.peer_instance, i);
		}
