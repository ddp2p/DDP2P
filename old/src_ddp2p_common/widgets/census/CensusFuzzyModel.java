...
public class CensusFuzzyModel extends AbstractTableModel implements TableModel,
	...
	public CensusFuzzyModel(int sign, long orgID) {
		Application.db.addListener(
				this,
				new ArrayList<String>(Arrays.asList(table.witness.TNAME,
						table.constituent.TNAME)), null);
		init(sign, orgID);
	}
	...
	boolean populateEdgeTable(long organizationID) {
		...
		if (!empty) {
			...
			try {
				...
				if (sqlResult != null) {
					maxRow = Integer.parseInt(util.Util.getString(sqlResult
							.get(0).get(0)));
				}
				sqlResult = Application.db.select(
				...
				for (ArrayList witnessStance : witness_stances) {
					sourceID = Integer.parseInt(util.Util
							.getString(witnessStance.get(0)));
					targetID = Integer.parseInt(util.Util
	...
	public void naiveMetric1(String pco, String nco, String t) {
		...
		try {
			...
			for (ArrayList rowData : sqlResult) {
				int constituentID = Integer.parseInt(util.Util
						.getString(rowData.get(0)));
				String value = util.Util.getString(rowData.get(3));
				String threshold = util.Util.getString(rowData.get(4));
