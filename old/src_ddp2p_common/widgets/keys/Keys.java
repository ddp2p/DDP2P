...
class KeysModel  extends AbstractTableModel implements TableModel, DBListener {
	...
	public void createPeer(int row) {
		D_Peer peer =
				D_Peer.getPeerByGID_or_GIDhash(Util.getString(_keydata.get(row).get(table.key.COL_PK)), null, true, true, true, null);
		if (peer.getLID() > 0) {
	...
	public Object getPeer(int row) {
		...
		String pk = Util.getString(_keydata.get(row).get(table.key.COL_PK));
	...
	*/
	static String sql_nohide = 
			...
			",c."+table.constituent.forename+
			",c."+table.constituent.name+
			",c."+table.constituent.organization_ID+//",oc."+table.organization.name+
			//",c."+table.constituent.constituent_ID+
			//",SUM(CASE c."+table.constituent.constituent_ID+" WHEN NULL 0 ELSE 1 END)"+
			" FROM "+table.key.TNAME+" AS k"+
			" LEFT JOIN "+table.constituent.TNAME+" AS c ON (c."+table.constituent.global_constituent_ID+"=k."+table.key.public_key+")"+
	...
	static int SELECT_COL_CONST_NB = table.key.FIELDS+0;
	static int SELECT_COL_CONST_FORENAME = table.key.FIELDS+1;
	static int SELECT_COL_CONST_NAME = table.key.FIELDS+2;
	static int SELECT_COL_CONST_ORG_ID = table.key.FIELDS+3;
	...
	public Object getConstituentsValue(int row) {
		...
		ArrayList<D_Constituent> cs = D_Constituent.getAllConstsByGID(Util.getString(_keydata.get(row).get(table.key.COL_PK)));
	...
	private Object s_getValueAt(int rowIndex, int columnIndex) {
		...
		case TABLE_COL_PEER:
		{
				String pk = Util.getString(_keydata.get(rowIndex).get(table.key.COL_PK));
				D_Peer peer = D_Peer.getPeerByGID_or_GIDhash(pk, null, true, false, false, null);
				if (peer == null) return null;
				return peer.getName_MyOrDefault();
		}
