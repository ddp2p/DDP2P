...
public class D_TesterIntroducer extends ASNObj implements Comparable<D_TesterIntroducer> {
	...
	public D_TesterIntroducer decode(Decoder dec) throws ASN1DecoderFail {
		...
		testerLID = data.D_Tester.getTesterLIDbyGID(testerGID);
		...
		private static final String sql_load_testerIntroducers = 
				"SELECT "+table.tester_introducer.fields_tester_introducer 
				+ " FROM "+table.tester_introducer.TNAME+
				  " WHERE "+table.tester_introducer.testerLID+"=?;";
		...
		private static D_TesterIntroducer buildRecord(ArrayList<Object> a) {
			...
			testerIntroducer.weight = Util.getString(a.get(table.tester_introducer.F_WEIGH));
			testerIntroducer._weight = Util.fval(testerIntroducer.weight, 0.0f);
			testerIntroducer.creation_date = Util.getCalendar(Util.getString(a.get(table.tester_introducer.F_CREATION_DATE)));
			testerIntroducer.introducerPeerLID = Util.lval(a.get(table.tester_introducer.F_INTRODUCER_PEER_LID), -1);
			testerIntroducer.testerLID = Util.lval(a.get(table.tester_introducer.F_TESTER_LID), -1);
			testerIntroducer.signature = Util.byteSignatureFromString(Util.getString(a.get(table.tester_introducer.F_SIGNATURE))); 
			testerIntroducer.testerRejectingDate = Util.getCalendar(Util.getString(a.get(table.tester_introducer.F_TESTER_REJECTING_DATE)));
			testerIntroducer.attackByIntroducer = Util.get_int(a.get(table.tester_introducer.F_TESTER_REJECTING_DATE));
...
//		}
		...
		public static void insertTesterIntroducerInfo(D_TesterIntroducer testerIntroducer) {
			String params[] = new String[table.tester_introducer.F_FIELDS_NOID];
			...
			params[table.tester_introducer.F_TESTER_REJECTING_DATE] = null;//Encoder.getGeneralizedTime(testerIntroducer.testerRejectingDate);
			params[table.tester_introducer.F_ATTACK_BY_INTRODUCER] = "0";//Util.getString(testerIntroducer.attackByIntroducer);
//			// ID:
//			params[table.tester_introducer.F_ID] = Util.getStringID(testerIntroducer.testerIntroducerID);
			
			try {
				Application.db.insert(table.tester_introducer.TNAME,
						table.tester_introducer._fields_tester_introducer_no_ID,
						params,
						DEBUG);
			} catch (P2PDDSQLException e) {
				e.printStackTrace();
		...
		public static void updateTesterIntroducer(
			...
			params[table.tester_introducer.F_WEIGH] = testerIntroducer.weight;
			params[table.tester_introducer.F_CREATION_DATE] = Encoder.getGeneralizedTime(testerIntroducer.creation_date);
			params[table.tester_introducer.F_TESTER_REJECTING_DATE] = Encoder.getGeneralizedTime(testerIntroducer.testerRejectingDate);
			params[table.tester_introducer.F_ATTACK_BY_INTRODUCER] = Util.getString(testerIntroducer.attackByIntroducer);
			// ID:
			params[table.tester_introducer.F_ID] = Util.getStringID(testerIntroducerID);
