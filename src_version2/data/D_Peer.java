...
/*
	...
	private void _init(ArrayList<Object> p, boolean encode_addresses, boolean encode_served_orgs) {
		...
		setCreationDateNoDirty(null, Util.getString(p.get(table.peer.PEER_COL_CREATION)));
		...
		this.setLID(Util.getString(p.get(table.peer.PEER_COL_ID)));
		...
		instance_null.set_last_sync_date_str(Util.getString(p.get(table.peer.PEER_COL_LAST_SYNC)));
		...
		setNameVerified(Util.stringInt2bool(Util.getString(p.get(table.peer.PEER_COL_VER_NAME)), false));
		setEmailVerified(Util.stringInt2bool(Util.getString(p.get(table.peer.PEER_COL_VER_EMAIL)), false));
	...
	private final static String sql_peer_org_inf = 
				...
				+ " FROM " + table.peer_org_inferred.TNAME
				+ " WHERE " + table.peer_org_inferred.COL_PEER_ID + " = ?;";
		...
		if (keep) {
			synchronized (monitor_object_factory) {
				...
				if (crt != null) {
					crt.incStatusLockWrite();
					if (crt.getStatusLockWrite() >  1) {
						Util.printCallPath("Why: GIDH="+GIDhash+" GID="+GID);
					}
				}
				return crt;
			}
...
//	}	
	...
	private void storeBasicComponent() throws P2PDDSQLException {
		...
		String params[];
		if (getLIDstr() == null) params = new String[table.peer.PEER_COL_FIELDS_NO_ID];
		...
		synchronized (dirty_main_monitor) {
			...
			params[table.peer.PEER_COL_SIGN]=getSignatureDB(); //Util.stringSignatureFromByte(getSignature());
			params[table.peer.PEER_COL_CREATION]=this.getCreationDate(); //Encoder.getGeneralizedTime(component_basic_data.creation_date);
			...
			params[table.peer.PEER_COL_VER_EMAIL]=Util.bool2StringInt(isEmailVerified());
			params[table.peer.PEER_COL_VER_NAME]=Util.bool2StringInt(isNameVerified());
		...
		if (getLIDstr() == null) {
			...
			params[table.peer.PEER_COL_ID] = getLIDstr();
	...
	private void storeMyPeerData() throws P2PDDSQLException {
		...
		else params = new String[table.peer_my_data.FIELDS];
		synchronized (this.dirty_my_data_monitor) {
			...
			params[table.peer_my_data.COL_PEER_ID] = getLIDstr();
	...
	private void storeServedOrgInferred() throws P2PDDSQLException {
		...
		if (this.served_org_inferred_orig != null) {
			for (D_PeerOrgInferred p_org_inf_orig : this.served_org_inferred_orig) {
				...
				if (pi == null) {
					Application.db.delete(table.peer_org_inferred.TNAME,
							new String[]{table.peer_org_inferred.peer_ID, table.peer_org_inferred.organization_ID},
	...
	public void forgetKeys() {
		try {
			Application.db.delete(table.key.TNAME, new String[]{table.key.public_key}, new String[]{this.getGID()}, DEBUG);
		} catch (P2PDDSQLException e) {
			e.printStackTrace();
		}
		this.keys = null;
	...
	*/
	...
	public void purge() {
    	...
    	try {
			...
			Application.db.deleteNoSync(table.peer_org_inferred.TNAME, new String[]{table.peer_org_inferred.peer_ID}, new String[]{peerID}, DEBUG);
	...
	static public void delete(D_Peer peer) throws P2PDDSQLException {
		synchronized(monitor_object_factory) {
			...
			Application.db.deleteNoSync(table.peer.TNAME, new String[]{table.peer.peer_ID}, new String[]{peer.getLIDstr_keep_force()}, DEBUG); // never loaded
	...
	public static ArrayList<ArrayList<Object>> getAllPeers(boolean order_by_used) {
		...
		try {
			String sql;
			sql = "SELECT "+table.peer.peer_ID+ 
					" FROM "+table.peer.TNAME;
			if (order_by_used)
				sql += " ORDER BY "+ table.peer.used+" DESC ";
			sql		+="; ";
...
//		this.dirty_instances = true;
	...
	final static String queryPeers_max2 =
					...
					",p."+table.peer.global_peer_ID_hash+
						...
						" AND ( p."+table.peer.arrival_date+" > ? )" +
							" AND ( p."+table.peer.arrival_date+" <= ? ) " +
						" GROUP BY p."+table.peer.peer_ID;
	...
	public void setPluginInfo(String instance, String plugin_info) {
		...
		if (!Util.equalStrings_null_or_not(inst.plugin_info, plugin_info)) {
			inst.plugin_info = plugin_info;
			inst.dirty = true;
			this.dirty_instances = true;
		}
	}
	public String getPluginInfo(String instance) {
		D_PeerInstance inst = this.getPeerInstance(instance);
		if (inst == null) return null;
		return inst.plugin_info;
	}
