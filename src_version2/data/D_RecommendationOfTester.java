...
public class D_RecommendationOfTester  implements  DDP2P_DoubleLinkedList_Node_Payload<D_RecommendationOfTester> {
	...
	private static final String sql_load_recOT = 
			"SELECT "+table.recommendation_of_tester.fields_recommendationOfTester 
			+ " FROM "+table.recommendation_of_tester.TNAME
			+ " WHERE "+table.recommendation_of_tester.senderPeerLID+"=? "
			+ " AND "+table.recommendation_of_tester.testerLID+"=?;";
	...
	private static final String sql_load_recOTBtLID = 
			"SELECT "+table.recommendation_of_tester.fields_recommendationOfTester 
			+ " FROM "+table.recommendation_of_tester.TNAME
			+ " WHERE "+table.recommendation_of_tester.recommendationID+"=?;";
	...
	private void init(ArrayList<Object> a) {
		this.weight = Util.getString(a.get(table.recommendation_of_tester.F_WEIGH));
		this._weight = Util.fval(this.weight, 0.0f);
		this.creation_date = Util.getCalendar(Util.getString(a.get(table.recommendation_of_tester.F_CREATION_DATE)));
		this.arrival_date  = Util.getCalendar(Util.getString(a.get(table.recommendation_of_tester.F_ARRIVAL_DATE)));
		this.address = Util.getString(a.get(table.recommendation_of_tester.F_ADDRESS));
		this.recommendationID = Util.lval(a.get(table.recommendation_of_tester.F_ID), -1);
		this.senderPeerLID = Util.lval(a.get(table.recommendation_of_tester.F_SENDER_PEER_LID), -1);
		this.testerLID = Util.lval(a.get(table.recommendation_of_tester.F_TESTER_LID), -1);
		this.signature = Util.byteSignatureFromString(Util.getString(a.get(table.recommendation_of_tester.F_SIGNATURE))); 
	}
...
//			}
	...
	void storeAct() throws P2PDDSQLException {
		...
		String params[] = new String[table.recommendation_of_tester.F_FIELDS];

		params[table.recommendation_of_tester.F_SENDER_PEER_LID] = Util.getStringID(this.senderPeerLID);
		params[table.recommendation_of_tester.F_TESTER_LID] = Util.getStringID(this.testerLID);
		params[table.recommendation_of_tester.F_WEIGH] = this.weight;
		params[table.recommendation_of_tester.F_ADDRESS] = this.address;
		params[table.recommendation_of_tester.F_CREATION_DATE] = Encoder.getGeneralizedTime(this.creation_date);
		params[table.recommendation_of_tester.F_SIGNATURE] = Util.stringSignatureFromByte(this.signature);
		params[table.recommendation_of_tester.F_ARRIVAL_DATE] = Encoder.getGeneralizedTime(this.arrival_date);
		params[table.recommendation_of_tester.F_ID] = Util.getStringID(this.recommendationID);
		
		
		if (update) {
			Application.db.update(table.recommendation_of_tester.TNAME, table.recommendation_of_tester._fields_recommendationOfTester_no_ID,
					new String[]{table.recommendation_of_tester.recommendationID},
					params,DEBUG);
		}else{
