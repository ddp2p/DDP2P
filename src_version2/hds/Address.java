...
public class Address extends ASNObj {
	...
	private void init(ArrayList<Object> in, D_Peer peer) {
		...
		branch = Util.getString(in.get(table.peer_address.PA_BRANCH));
		agent_version = Util.getString(in.get(table.peer_address.PA_AGENT_VERSION));
			...
			if (dpi != null) {
				...
				try {
					...
					ArrayList<ArrayList<Object>> inst = Application.db.select(
							"SELECT "+table.peer_instance.peer_instance+
							" FROM "+table.peer_instance.TNAME+
							" WHERE "+table.peer_instance.peer_instance_ID+"=?;",
	...
	public void store(String peer_ID, String instance_ID) throws P2PDDSQLException {
		...
		if (this.isDIR() || this.instance_ID_str == null) {
			params[table.peer_address.PA_BRANCH] = branch;
			params[table.peer_address.PA_AGENT_VERSION] = agent_version;
		}
		...
		if (peer_address_ID == null) {
			peer_address_ID = ""+Application.db.insert(table.peer_address.TNAME, 
					table.peer_address.fields_noID_list, params, DEBUG);
		} else {
