...
class Decoder {
	...
	public BigInteger getTagValueBN() {
		...
		//BigInteger bn128 = Util.BN128;
		return Util.fromBase128(data, offset + 1, offset + length);
	}
	public int typeLen(){
		if (length <= 0) return 0;
		...
		int k = offset+1;
		int len = 2;
		while ((k < length+offset) && (Util.getUnsignedShort(data[k]) > 127)) {len++; k++;}
	...
	public int contentLength() {
		...
		if (Util.getUnsignedShort(data[offset+tlen]) < 128) {
		...
		int len_len=Util.getUnsignedShort(data[offset+tlen])-128;
	...
	public int lenLen(){
		...
		if(Util.getUnsignedShort(data[offset+tlen])<128) return 1;
		return 1+Util.getUnsignedShort(data[offset+tlen])-128;
	}
	/**
	 * 
...
@return	A new Encoder
	...
	public boolean getBoolean() {
		...
		if (!(data[offset]==Encoder.TAG_BOOLEAN))
			Util.printCallPathTop("Encoder: getBoolean: Failed Encoder:"+getTypeByte());
	...
	public BigInteger getInteger() {
		...
		if (DEBUG) System.out.println("Decoder: getInteger: result = "+Util.byteToHex(data, offset, typeLen()+lenLen(), " "));
	...
	public byte[] getBytes() {
		...
		if (!((data[offset]==Encoder.TAG_OCTET_STRING)||
			...
			Util.printCallPathTop("Encoder: getBytes: Failed Encoder:"+getTypeByte());
		}
		if(getTypeByte()==Encoder.TAG_NULL) return null;
		byte[] value = new byte[contentLength()];
		Encoder.copyBytes(value, 0, data, contentLength(),offset+typeLen()+lenLen());
	...
	public BigInteger[] getBNOID(byte type) throws ASN1DecoderFail {
		...
		for (int k = 2, crt = 1; k < value.length; k ++) {
			//value[k] = Util.fromBase128(b_value, k, this.offset + this.length);
	...
	public String getString() {
		...
		if(!((data[offset]==Encoder.TAG_IA5String)||
			...
			Util.printCallPathTop("Encoder: getString: Failed Encoder:String type:"+getTypeByte());
...
//		}
	...
	public static void encodeDecodeBNTAG(String nb) throws ASN1DecoderFail {
		...
		msg = enc.getBytes();
		if (DEBUG) System.out.println("BN: "+Util.byteToHex(msg));
