...
public class D_PeerOrgInferred {
	...
	public D_PeerOrgInferred(ArrayList<Object> ipoi) {
		peer_org_inferred_ID = Util.lval(ipoi.get(table.peer_org_inferred.COL_ID));
		peer_ID = Util.lval(ipoi.get(table.peer_org_inferred.COL_PEER_ID));
		organization_ID = Util.lval(ipoi.get(table.peer_org_inferred.COL_ORG_ID));
	}
	...
	public void store(D_Peer d_Peer) throws P2PDDSQLException {
		...
		dirty = false;
		String[] params = new String[table.peer_org_inferred.fields_list.length];
		params[table.peer_org_inferred.COL_PEER_ID] = d_Peer.getLIDstr();
		params[table.peer_org_inferred.COL_ORG_ID] = Util.getStringID(organization_ID);
		if (peer_org_inferred_ID > 0) {
			params[table.peer_org_inferred.COL_ID] = Util.getStringID(peer_org_inferred_ID);
			Application.db.update(table.peer_org_inferred.TNAME,
					table.peer_org_inferred.fields_no_ID_list,
					new String[]{table.peer_org_inferred.peer_org_inferred_ID},
					params,
					DEBUG);
		} else {
			peer_org_inferred_ID = Application.db.insert(table.peer_org_inferred.TNAME,
					table.peer_org_inferred.fields_list,
					params,
					DEBUG);
		}
