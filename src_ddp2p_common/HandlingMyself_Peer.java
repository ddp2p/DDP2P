...
public class HandlingMyself_Peer {
	...
	*/
	...
	static public boolean setMyself( D_Peer me, boolean saveInDB, Identity my_ID) {
		...
		synchronized (monitor_init_myself) {
			...
			synchronized(_myself_monitor) {
				...
				if (me != null) me = D_Peer.getPeerByGIDhash(me.getGIDH_force(), true, true, false);
				_myself = me;
	...
	static public D_Peer getPeer(Identity id) {
		...
		D_Peer me = D_Peer.getPeerByGID_or_GIDhash_NoKeep(id.globalID, true, false, null);
		if (me == null) {
...
//		}
	...
	public static D_Peer createPeerUnsigned_Keep(PeerInput peerInput) throws P2PDDSQLException{
		...
		if (addresses == null) addresses = __("LocalMachine");
