...
//		}catch(IOException _IoExc) { QuitConnection(QUIT_TYPE_DEFAULT);}			
    ...
    public int NumberOfpeerInUserCanvas(){
		return tappanel.UserCanvas.getListArraySize();
	}
	...
	public void addMsgs(List<ChatEntity> listEntity){
		if(listEntity==null){
			System.out.println("error, why listEntity is null");
			return;
		}

		for(int i=0; i<listEntity.size(); i++){
			if(listEntity.get(i)==null){
				System.out.println("ChatPeer: entity is null");
				continue;
			}
			if(listEntity.get(i).getMsg()==null){
				System.out.println("ChatPeer: msg is null");
				continue;
			}
			messagecanvas.AddMessageToMessageObject(UserRoomGID, listEntity.get(i).getMsg().session_id, listEntity.get(i).getMsg().sequence, 
				                                    listEntity.get(i).getContent(), MESSAGE_TYPE_DEFAULT);
		}
	}
	/*********Button Events *****/
...
//		 Main.enqueue(msg);
		...
		messagecanvas.AddMessageToMessageObject(null, null, null, UserName+": "+TxtMessage.getText()+" --not yet sent",MESSAGE_TYPE_DEFAULT);	
		TxtMessage.setText("");
