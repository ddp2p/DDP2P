...
//		System.out.println(cnt+" "+cnt.testBit(0));
	...
	public void init() {
		...
		if((observer>0)&&(observer<=n)){
			for(int a=1 ; a<=n; a++){
				for(int c=1; c<=C; c++){
					if((this.witness[observer][a][c]!=_) && (getObserverCPT(observer,a,c)==0)) {
	...
	private void mcmc_A_RW(int a) {
		...
		for(int b=1; b<=n; b++) {
			...
			if(a!=b) {
				...
				if(witness[a][a][CS]!=_) {// a summation for Waa_CS
					if(witness[a][a][CS]==T) {
				...
				if(witness[a][a][RW]!=_) {// a summation for Waa_CS
					if(witness[a][a][RW]==T) {
...
//	        System.out.println("size:"+size);
	        ...
	        while (hasNextLine(s)) {
	            ...
	            if(split.length==4){
	            	...
	            	Application.warning("Bad work!", "Bad work!");
	            	return null;
	            }
//	            if(split.length==2){
//	                int i=Integer.parseInt(split[0]);
