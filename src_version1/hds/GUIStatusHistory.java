...
class GUIStatusHistory implements OrgListener, MotionsListener, JustificationsListener, PeerListener, ConstituentListener{
	private final class DispatcherOrg extends Thread {
		...
		public void run(){
			...
			l.orgUpdate(org.organization_ID, Orgs.A_NON_FORCE_COL, org);
	...
	public void fireOrgListeners() {
		...
		for(OrgListener l: org_listeners) {
			if(DEBUG) System.out.println("GUIStatus: fireListener: l="+l);
			try{
				if(org==null) l.orgUpdate(null, Orgs.A_NON_FORCE_COL, org);
				else l.orgUpdate(org.organization_ID, Orgs.A_NON_FORCE_COL, org);
			}catch(Exception e){e.printStackTrace();}
		}
	}
...
//		Util.printCallPath("stack");
	...
	static public boolean samePeer(D_PeerAddress c, D_PeerAddress d) {
	...
	synchronized
	D_PeerAddress getSelectedPeer(int prev) {
		if(!checkIdx(prev)) return null;
		return history.get(prev).getSelectedPeer();
	}
	synchronized
	D_PeerAddress getMePeer(int prev) {
		if(!checkIdx(prev)) return null;
...
class GUIStatus {
	...
	D_PeerAddress myself_peer;
	D_PeerAddress selected_peer;
	...
	void setMePeer(D_PeerAddress peer){
		backtrack(0);
		myself_peer = peer;
	}
	D_PeerAddress getMePeer(){
