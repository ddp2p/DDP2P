...
import handling_wb.BroadcastQueueCircular;
				...
				for(int i=0; i<addresses.length; i++) {
					...
					try {
						...
						try {
							...
							if(adr==null){
								Application.warning(Util._("Trouble retrieving interface:")+addresses[i], Util._("Cannot use interface"));
								continue;								
							}
							ia = InetAddress.getByAddress(adr);
						} catch (UnknownHostException e) {
							e.printStackTrace();
							Application.warning(Util._("Trouble retrieving interface:")+e.getLocalizedMessage(), Util._("Cannot use interface"));
							continue;
						}
						...
						e.printStackTrace();
						Application.warning(e.getLocalizedMessage(), Util._("Failure to get interface"));
						//System.exit(1);
					}
				}
			}
	...
	public void run() {
		...
		for(int i=0;;i++) {
			...
			EventQueue.invokeLater(new Runnable(){
				public void run(){
					DD.client_sockets.setText(""+wireless.BroadcastClient.broadcast_client_sockets.length);
					String t = "";
					...
					DD.client_sockets_val.setText(t);
				}
			});
			...
			for(int j=0; j<broadcast_client_sockets.length; j++){
				...
				try {
					...
					*/
					...
					EventQueue.invokeLater(new Runnable(){
						public void run(){
							DD.client_sockets_cntr.setText(""+msg_cnter);
							String t = "";
							for(int i = 0; i<wireless.BroadcastClient.broadcast_client_sockets.length; i++) {
								if(i>0) t+= " ;; ";
								t += wireless.BroadcastClient.broadcast_client_sockets[i].getLocalSocketAddress();
