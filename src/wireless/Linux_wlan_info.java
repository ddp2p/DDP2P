...
public class Linux_wlan_info {
	...
	public static ArrayList<InterfaceData> process() throws IOException{
		...
		BufferedReader bri1 = Util.getProcessOutput(cmd_names);
		if(bri1!=null){
		...
		for(int i=0;i<interface_names.size();i++){
			...
			BufferedReader bri2 = Util.getProcessOutput(cmd_ssid); // new BufferedReader (new InputStreamReader(p2.getInputStream()));
		...
		for(int i=0;i<interface_names.size();i++){
			...
			BufferedReader bri3 = Util.getProcessOutput(cmd_ip); //new BufferedReader (new InputStreamReader(p3.getInputStream()));
