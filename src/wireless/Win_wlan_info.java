...
public class Win_wlan_info {
	...
	public static String get_output_ADHOC_DD_IP_WINDOWS_NETSH_INTERFACE_IDENTIFIER(String interf){
		...
		try {
			BufferedReader bri = Util.getProcessOutput(s); 
	...
	public static InterfaceData get_ip_from_ipconfig(Object key, String ssid) throws IOException {
		...
		String s="ipconfig";		
		BufferedReader bri = Util.getProcessOutput(new String[]{s}); //new BufferedReader (new InputStreamReader(new ByteArrayInputStream(ipconfig_result.getBytes())));
		if(bri!=null){
			while ((line = bri.readLine()) != null) {
				if(line.length()!=0)
					ipconfig.add(line);
		...
		for(int i=0;i<ipconfig1.size();i++){
			...
			if(flag==0) id.IP = Util._(Detect_interface.WINDOWS_NO_IP);
		}
	...
	public static String get_output_ADHOC_DD_IP_WINDOWS_NETSH_IP_IDENTIFIER(String interf){
		...
		try {
			BufferedReader bri = Util.getProcessOutputConcatParams(s); 
			return Util.readAll(bri);
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	...
	public static InterfaceData get_ip_from_netsh(Object key) throws IOException {
		...
		BufferedReader bri = Util.getProcessOutputConcatParams(s); //new BufferedReader (new InputStreamReader(new ByteArrayInputStream(ipconfig_result.getBytes())));
		if(bri!=null){
			while ((line = bri.readLine()) != null) {
				if(line.length()!=0)
					netsh_ip.add(line);
		...
		for(int i=0;i<netsh_ip.size();i++){
			...
			if(netsh_ip.get(i).contains(Wireless_NETSH))
			{
				...
				if(flag==0) { Ip=Detect_interface.WINDOWS_NO_IP;  id.IP = Ip;//System.out.println("IP : "+Ip); 
				}
			}
			id.SSID = DD.DD_SSID;
	...
	public static DatagramSocket[] extractValidIPs(BroadcastData _BD) {
		...
		for(int i=0;i<_BD.Interfaces_names.length;i++){
			try {
				...
				if(!Detect_interface.WINDOWS_NO_IP.equals(id.IP))
