...
import widgets.constituent.ConstituentListener;
...
public class StatusBar extends JPanel implements OrgListener, MotionsListener, MouseListener, PeerListener, ConstituentListener, ActionListener{
	...
	StatusBar(GUIStatusHistory status) {
		super(new FlowLayout(FlowLayout.LEADING, 10, 0));
		//this.setMinimumSize(new Dimension(100,100));
		this.status = status;
		available_labels = new String[]{_("Myself (Peer)"),_("Selected Peer"),_("Identity"),_("Organization"),
				_("Myself (Constituent)"),_("Selected Constituent"),_("Motion"),_("Language")};
	...
	public void connectWidget() {
		DD.status.addPeerMeStatusListener(this);
		DD.status.addPeerSelectedStatusListener(this);
		DD.status.addOrgStatusListener(this);
		DD.status.addConstituentMeStatusListener(this);
		DD.status.addConstituentSelectedStatusListener(this);
	...
	void init_components() {
		...
		this.add(back=new JButton(DDIcons.getBackImageIcon(_("Back"))));
		this.add(forward=new JButton(DDIcons.getForwImageIcon(_("Forward"))));
		back.setToolTipText(_("Back"));
		forward.setToolTipText(_("Forward"));
	...
	public void motion_update(String motID, int col, D_Motion d_motion) {
		String text;
		if(d_motion != null) text = Util.trimmed(d_motion.motion_title.title_document.getDocumentUTFString(), 50);
		else text = "";
		motion.setText(text);
	}
	@Override
	...
	public void configure(int def){
		int k = Application.ask(_("What to Toggle"), _("Toggle the fields:"), available_labels, available_labels[def], null);
	...
	public void actionPerformed(ActionEvent a) {
		int lastTab = DD.status.getTab();
		if("BACK".equals(a.getActionCommand())) DD.status.back();
		if("FORWARD".equals(a.getActionCommand())) DD.status.forward();
		int crtTab = DD.status.getTab();
		if ((crtTab != lastTab) && (crtTab > 0))
			DD.tabbedPane.setSelectedIndex(crtTab);
	}
}
