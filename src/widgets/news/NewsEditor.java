...
public class NewsEditor  extends JPanel  implements NewsListener, DocumentListener, ItemListener, ActionListener, OrgListener, MotionsListener, MouseListener {
	...
	boolean update_it(boolean force) {
		...
		try {
			if(organization_ID>0)org_label.setText(new D_Organization(organization_ID).name);
			else org_label.setText("");
			if(motion_ID>0)mot_label.setText(new D_Motion(motion_ID).motion_title.title_document.getDocumentUTFString());
		...
		String sql =
			"SELECT "+table.news.title+
			","+table.news.global_news_ID+
			","+table.news.news_ID+
	...
	JPopupMenu getPopup(int row, int col){
    	...
    	aAction = new NewsEditorCustomAction(this, _("Delete Organization Restriction!"), delicon,_("Delete organization restriction."), _("Delete Organization Restriction"),KeyEvent.VK_O, NewsEditorCustomAction.M_DELORG);
    	aAction.putValue("row", new Integer(row));
    	menuItem = new JMenuItem(aAction);
    	popup.add(menuItem);
    	aAction = new NewsEditorCustomAction(this, _("Delete Motion Restriction!"), delicon,_("Delete motion restriction."), _("Delete Motion Restriction"),KeyEvent.VK_M, NewsEditorCustomAction.M_DELMOT);
    	aAction.putValue("row", new Integer(row));
    	menuItem = new JMenuItem(aAction);
    	popup.add(menuItem);
    	return popup;
