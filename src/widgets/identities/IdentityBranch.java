...
public class IdentityBranch extends IdentityNode{
    ...
    public IdentityBranch(MyIdentitiesModel _model, String _name, int _nchildren, long _identityID, 
    	...
    	try {
			id=Application.db.select("SELECT c."+table.constituent.global_constituent_ID+", o."+table.organization.global_organization_ID+", o."+table.organization.name+", c."+table.constituent.name+", c."+table.constituent.forename +
			...
			globalID = (String) id.get(0).get(0);
			globalOrgID = (String) id.get(0).get(1);
    ...
    public void del(Object child) {
    	...
    	try{
    		...
    		JOptionPane.showMessageDialog(null,_("Error deleting child:")+e.toString());
    		e.printStackTrace();
    	}
    }
    ...
    public String toString() {
    	String result;
    	if(nchildren>0)
    		result =  MessageFormat.format(_("{0} ({1} properties)"), new Object[]{name,nchildren});
    	else
    ...
    public String getTip() {
    	String result;
    	result = this.tip;
    	if(this==Identity.current_id_branch) result = result + " " + _("Current!");
    	if(default_id) result = result + " " + _("Default!");
    	if (this.getKeys()==null) result = result + " " + _("No Keys!");
    	return result;
    }
