...
public class Motions extends JTable implements MouseListener, MotionsListener  {
	...
	void init(){
		...
		Comparator<D_Document_Title> documentTitleComparator = new java.util.Comparator<D_Document_Title>() {
			public int _compare(Object o1, Object o2) {
				...
				if(o1 instanceof net.ddp2p.common.data.D_Document_Title) s1 = ((net.ddp2p.common.data.D_Document_Title)o1).title_document.getDocumentUTFString();
	...
	public TableCellRenderer getCellRenderer(int row, int column) {
		...
		case MotionsModel.TABLE_COL_VOTERS_NB: 
		case MotionsModel.TABLE_COL_ACTIVITY: 
		case MotionsModel.TABLE_COL_NEWS: 
			return centerRenderer;
		case MotionsModel.TABLE_COL_RECENT: 
			return hotRenderer;
		case MotionsModel.TABLE_COL_BROADCASTED: 
		case MotionsModel.TABLE_COL_BLOCKED: 
		case MotionsModel.TABLE_COL_TMP: 
		case MotionsModel.TABLE_COL_GID_VALID: 
		case MotionsModel.TABLE_COL_SIGN_VALID: 
		case MotionsModel.TABLE_COL_HIDDEN: 
			return super.getCellRenderer(row, column);
		case MotionsModel.TABLE_COL_CREATOR: 
		case MotionsModel.TABLE_COL_CATEGORY: 
		case MotionsModel.TABLE_COL_CREATION_DATE: 
		default:
	...
	public void mouseClicked(MouseEvent evt) {
    	...
    	MotionsModel model = (MotionsModel)getModel();
 		...
 		if (DEBUG) System.out.println("Motions: mouseClicked: model_row="+model_row);
   	   	if (model_row >= 0) {
   	   		long motID = model.getMotionID(model_row); 
...
class MotionCustomAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if (cmd == M_REFRESH){
    		if (DEBUG) System.out.println("Motions:MotionCustomAction: Redraw: start");
    		new net.ddp2p.common.util.DDP2P_ServiceThread(__("Motions Refresh"), true, tree) {
				@Override
				public void _run() {
					Motions tree = (Motions) ctx; 
    	...
    	if (cmd == M_TOGGLE_HIDE) {
    		...
    		new net.ddp2p.common.util.DDP2P_ServiceThread("Motions Hide", true, model) {
				@Override
				public void _run() {
        ...
        if (cmd == M_REM_ENHANCING) {
       	...
       	if (cmd == M_EXPORT) {
			...
			D_Motion motion = model.getMotion(row); 
			...
			D_Organization org = motion.getOrganization();
			filterUpdates.setFileFilter(new net.ddp2p.widgets.components.StegoFilterKey());
			...
			try {
				...
				dsk.sign_and_set_sender(net.ddp2p.common.data.HandlingMyself_Peer.get_myself_or_null()); 
