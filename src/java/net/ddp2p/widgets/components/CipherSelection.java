...
class CipherSelection extends JPanel implements DocumentListener, ActionListener {
	...
	void init() {
	    add(crt_cipher = new JComboBox<String>(net.ddp2p.ciphersuits.Cipher.getAvailableCiphers()));
	...
	void setSelectedCipher(String cipher) {
		...
		Cipher_Sizes cs = net.ddp2p.ciphersuits.Cipher.getAvailableSizes(cipher);
		if (cs == null) {
	...
	private void cipherSizeChanged () {
		int size = this.getCrtCipherSize(); 
