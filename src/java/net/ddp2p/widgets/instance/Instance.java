...
class KeysModel  extends AbstractTableModel implements TableModel, DBListener {
	...
	public KeysModel(DBInterface _db) {
		init_statics();
		db = _db;
		db.addListener(this, new ArrayList<String>(Arrays.asList(net.ddp2p.common.table.peer_instance.TNAME)), null);
		update(null, null);
	}
	public void createPeer(int row) {
		D_Peer peer =
				D_Peer.getPeerByGID_or_GIDhash(Util.getString(_instancedata.get(row).get(net.ddp2p.common.table.key.COL_PK)), null, true, true, true, null);
		if(peer.getLID() > 0){
	...
	private void set_my_data(String field_name, String value, int row) {
		...
		String _ID = Util.getString(_instancedata.get(row).get(net.ddp2p.common.table.key.COL_ID));
		try {
