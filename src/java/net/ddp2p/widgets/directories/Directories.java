...
public class Directories extends JTable implements MouseListener, Directories_View {
	...
	protected String[] columnToolTips = {null,null,__("A name you provide"),
			__("Branch"), __("Latest Supported Version"),
			__("Is it behind a NAT?"), __("Does it offer relay service"),
			__("UDP answered"), __("TCP answered"), __("UDP answered"),
			__("Served address"), __("Date last answer"), __("Use?")};
    @SuppressWarnings("serial")
...
class DirectoriesCustomAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if (command == C_REFRESH) {
        	...
        	new net.ddp2p.common.util.DDP2P_ServiceThread("Directory Announcement Reset", false) {
        		public void _run() {
		    		UDPServer.announceMyselfToDirectoriesReset();
		    		Application_GUI.warning(__("Announcing:")+UDPServer.directoryAnnouncement, __("Announcement to Listing Directories"));
        		}
        	};
        } else	if (command == C_ACOLUMN) {
        	int col = Application_GUI.ask(__("Add"), __("Columns"), Arrays.copyOf(DirectoriesModel.columnNames, DirectoriesModel.columnNames.length, new Object[]{}.getClass()), null, null);
...
class DirectoryPing extends net.ddp2p.common.util.DDP2P_ServiceThread {
				...
				for (int k = 0; k < _addr.length; k++) {
					...
					if (s == null) {
						...
						if (ia == null) {
							...
							m.addressByIPport.put(iport, __("Unreachable"));
			...
			for (;;) {
				...
				try {
					...
					if (!first_warning_done_dir_answer) {
						...
						Application_GUI.warning(__("Unsupported Directory Answer message: ")+
								e.getLocalizedMessage()+"\n"+sock_addr,
								__("Directory Answer fail"));
					}
					e.printStackTrace();
					socket.close();
					return null;
...
class DirectoriesModel extends AbstractTableModel implements TableModel, DBListener {
	...
	static String columnNames[]={__("IP"),__("Port"),__("Name"), __("Branch"),__("Version"),
			__("NAT Piercing"),__("Relay"),
			__("UDP"),__("TCP"),
			__("My address"), __("Date"), __("Active")};
	 /**
	...
	*/
	...
	public void setValueAt(Object value, int row, int col) {
		...
		*/
		...
		switch (col) {
		case COL_IP:
			ok = Application_GUI.ask(__("Are you sure you want to change the domain?"), __("Change IP"), JOptionPane.OK_CANCEL_OPTION);
			if (ok == 0) adr.setDomain(Util.getString(value));
			break;
		case COL_PORT_TCP:
			ok = Application_GUI.ask(__("Are you sure you want to change the port?"), __("Change Port"), JOptionPane.OK_CANCEL_OPTION);
