 ...
 * OrgFilter ::= SEQUENCE {
 *  version UTF8String,
 *  orgGID [Application 2] IMPLICIT PrintableString OPTIONAL,
 *  orgGID_hash [Application 3] IMPLICIT PrintableString OPTIONAL,
 *  last_sync_date [Application 3] IMPLICIT GeneralizedTime OPTIONAL,
 *  motions SEQUENCE OF [PRIVATE 4] IMPLICIT PrintableString OPTIONAL,
 *  motions_excluding BOOLEAN OPTIONAL,
 *  plugins SEQUENCE OF PrintableString OPTIONAL,
 *  plugins_excluding BOOLEAN OPTIONAL,
 * }
 */
public
class OrgFilter extends ASNObj{
	 ...
	 * OrgFilter ::= SEQUENCE {
	 *  version UTF8String,
	 *  orgGID [Application C2] IMPLICIT PrintableString OPTIONAL,
	 *  orgGID_hash [Application 3C] IMPLICIT PrintableString OPTIONAL,
	 *  last_sync_date GeneralizedTime OPTIONAL,
	 *  motions SEQUENCE OF [PRIVATE P4] IMPLICIT PrintableString OPTIONAL,
	 *  motions_excluding BOOLEAN OPTIONAL,
	 *  plugins SEQUENCE OF PrintableString OPTIONAL,
	 *  plugins_excluding BOOLEAN OPTIONAL,
	 * }
	...
	public OrgFilter decode(Decoder decoder) throws ASN1DecoderFail {
		...
		if(dec.getTypeByte()==Encoder.TYPE_SEQUENCE) {
			...
			for(;;) {
				Decoder c_m = d_m.getFirstObject(true, DD.TAG_PP0);
				if(c_m == null) break;
				motions.add(c_m.getString(DD.TAG_PP4));
			}
		...
		if(dec.getTypeByte()==DD.TAG_AC2) {
			...
			for(;;) {
				...
				if(c_m == null) break;
				plugins.add(c_m.getString(DD.TAG_PP4));
			}
