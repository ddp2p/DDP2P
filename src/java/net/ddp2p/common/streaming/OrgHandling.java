...
public class OrgHandling {
	...
	public	static boolean updateOrg(ASNSyncPayload asa, D_Organization od, String[] _orgID, String arrival_time,
			...
			)
		{
			...
			id = D_Organization.storeRemote(od, _changed, _sol_rq, _new_rq, peer); 
			if (DEBUG || DD.DEBUG_PRIVATE_ORGS || id <= 0)System.out.println("OrgHandling:updateOrg: org changed: ch="+_changed[0]+
					" br="+od.broadcast_rule+" id="+id+" creat="+od.creator_ID);
			if (DEBUG || id <= 0) System.out.println("OrgHandling:updateOrg: failed saving org="+od);
			if (
					...
					if (DD.WARN_OF_FAILING_SIGNATURE_ONRECEPTION) {
						...
						Application_GUI.warning(
								__("Verification/Storage failure for incoming org:")+od.getName()+"\n"+
										...
										__("You can disable such warnings from the Control Panel"),
	...
	public static String getNextOrgDate(String last_sync_date, String _maxDate, HashSet<String> orgs, int limitOrgLow) throws P2PDDSQLException {
		...
		if(_maxDate == null) {
			...
			" WHERE "+net.ddp2p.common.table.organization.arrival_date+" > ? " +
			...
			sql = "SELECT "+net.ddp2p.common.table.organization.arrival_date+", COUNT(*), " + net.ddp2p.common.table.organization.global_organization_ID+","+ net.ddp2p.common.table.organization.organization_ID+
					" FROM "+net.ddp2p.common.table.organization.TNAME + " AS o "+
				" WHERE "+net.ddp2p.common.table.organization.arrival_date+" > ? AND "+net.ddp2p.common.table.organization.arrival_date+" <= ? " +
				" AND " +net.ddp2p.common.table.organization.signature+ " IS NOT NULL "+
				 " AND o."+net.ddp2p.common.table.organization.broadcasted+" <> '0' " +
						" GROUP BY "+net.ddp2p.common.table.organization.arrival_date+","+net.ddp2p.common.table.organization.global_organization_ID+" " +
								" ORDER BY "+net.ddp2p.common.table.organization.arrival_date+" LIMIT "+(1+limitOrgLow)+";";
	...
	private static String getNextFilteredOrgDate(String last_sync_date, OrgFilter ofi, HashSet<String> orgs) throws P2PDDSQLException {
		...
		String sql, result=null;
		ArrayList<ArrayList<Object>>p_data;
		sql = "SELECT "+net.ddp2p.common.table.organization.arrival_date+", COUNT(*) FROM "+net.ddp2p.common.table.organization.TNAME + " AS o "+
			" WHERE ( ( "+net.ddp2p.common.table.organization.global_organization_ID+" = ? ) OR ( "+net.ddp2p.common.table.organization.global_organization_ID_hash+" = ? ) ) " +
			 " AND o."+net.ddp2p.common.table.organization.broadcasted+" <> '0' " +
	...
	public static D_Organization[] getOrgData(ASNSyncRequest asr, String last_sync_date, String[] _maxDate, 
		...
		if(asr.orgFilter==null) {
			...
			String sql = "SELECT " + net.ddp2p.common.table.organization.field_list +
				" FROM "+net.ddp2p.common.table.organization.TNAME+ " AS o " +
				" WHERE "+net.ddp2p.common.table.organization.arrival_date+" > ? "+
					((maxDate!=null)?("AND "+net.ddp2p.common.table.organization.arrival_date+" <= ? "):"") +
					" AND " +net.ddp2p.common.table.organization.global_organization_ID+ " IS NOT NULL "+
					 " AND o."+net.ddp2p.common.table.organization.broadcasted+" <> '0' " +
					" AND " +net.ddp2p.common.table.organization.signature+ " IS NOT NULL "+
			...
			for(int k=0; k<p_data.size(); k++) {
				...
				String gid = Util.getString(a.get(net.ddp2p.common.table.organization.ORG_COL_GID));
				...
				String id = Util.getString(a.get(net.ddp2p.common.table.organization.ORG_COL_ID));
			...
			do{ 
				...
				if (K != null) {
					int k = K.intValue();
					crt_org = D_Organization.getOrgByGID_or_GIDhash_NoCreate(org_gid, null, true, false); 
					_orgData.add(crt_org);
					...
					D_Organization all;
					all = D_Organization.getOrgByGID_or_GIDhash_NoCreate(org_gid, null, true, false); 
			...
			for(int of=0; of<asr.orgFilter.length; of++) {
				...
				String sql = "SELECT " + net.ddp2p.common.table.organization.field_list +
						...
						" WHERE ( "+net.ddp2p.common.table.organization.global_organization_ID+" = ? OR "+net.ddp2p.common.table.organization.global_organization_ID_hash+" = ? ) " +
								" ORDER BY "+net.ddp2p.common.table.organization.arrival_date+" LIMIT 100;";
				ArrayList<ArrayList<Object>>p_data =
					Application.getDB().select(sql, new String[]{ofi.orgGID, ofi.orgGID_hash}, DEBUG);
				if(p_data.size()>=1) {
					D_Organization od = 
					...
					_orgData.add(od);
					String org_id = Util.getString(p_data.get(0).get(net.ddp2p.common.table.organization.ORG_COL_ID));
					if(!OrgHandling.serving(asr, org_id)) continue;
					if(SERVE_DIRECTLY_DATA) {
						if(DEBUG) out.println("OrgHandling:getOrgData: SERVE_DIRECTLY");
						...
						if(DEBUG) out.println("OrgHandling:getOrgData: SERVE_INDIRECTLY:"+_maxDate[0]);
						RequestData rq = new RequestData();
						rq.global_organization_ID_hash = ofi.orgGID_hash; 
