...
class GUIStatusHistory implements OrgListener, MotionsListener, JustificationsListener, PeerListener, ConstituentListener{
	private final class DispatcherOrg extends Thread {
		...
		public void run(){
			...
			l.orgUpdate(org.getLIDstr(), D_Organization.A_NON_FORCE_COL, org);
	...
	public void fireOrgListeners() {
		...
		for(OrgListener l: org_listeners) {
			...
			try{
				...
				else l.orgUpdate(org.getLIDstr(), D_Organization.A_NON_FORCE_COL, org);
			}catch(Exception e){e.printStackTrace();}
		}
	}
	...
	public void fireMotListeners() {
		...
		for(MotionsListener l: mot_listeners) {
			...
			try{
				...
				else l.motion_update(mot.getLIDstr(), 0, mot);
			}catch(Exception e){e.printStackTrace();}
		}
	}
	public void addMotionStatusListener(MotionsListener l) {
		...
		if(mot != null){
			...
			l.motion_update(mot.getLIDstr(), 0, mot);
		}
	}
	...
	void setSelectedConstituent(D_Constituent c){
		if(DEBUG) System.out.println("GUIStatus: setSelectedConst "+((c==null)?"null":c.getNameFull()));
	...
	public void setMeConstituent(D_Constituent c) {
		if(DEBUG) System.out.println("GUIStatus: setMeConstituent "+((c==null)?"null":c.getNameFull()));
	...
	void setSelectedMot(D_Motion mot){
		if(DEBUG) System.out.println("GUIStatusHistory: setMots "+((mot==null)?null:mot.getMotionTitle()));
	...
	void setSelectedJust(D_Justification just, boolean sync){
		if(DEBUG) System.out.println("GUIStatusHistory: setJusts "+((just==null)?null:just.getJustificationTitle()));
		...
		cleanForward();
		pushStack();
		if(DEBUG) System.out.println("GUIStatusHistory: setJusts set to="+((just==null)?null:just.getJustificationTitle()));
	...
	static public boolean samePeer(D_Peer c, D_Peer d) {
		...
		return (c.getLID()==d.getLID());
	}
	static public boolean sameConstituent(D_Constituent c, D_Constituent d) {
		if((c==null)&&(d==null)) return true;
		if((c==null)||(d==null)) return false;
		if((c.getLIDstr()==null)&&(d.getLIDstr()==null)) return true;
		if((c.getLIDstr()==null)||(d.getLIDstr()==null)) return false;
		return c.getLIDstr().equals(d.getLIDstr());
	}
	...
	private void tellListeners(int crt, int previous) {
		...
		if(mot!=m_prev){
			if (DEBUG) System.out.println("GUISH: tellListeners: new mot "+((mot==null)?"null":mot.getMotionTitle().title_document.getDocumentUTFString()));
			fireMotListeners();
		}
		D_Constituent selcon = getSelectedConstituent();
		D_Constituent sc_prev = getSelectedConstituent(previous);
		if(selcon!=sc_prev){
			if (DEBUG) System.out.println("GUISH: tellListeners: new selcon "+((selcon==null)?"null":selcon.getNameFull()));
			fireConstituentSelectedListeners();
		}
		D_Constituent mecon = getMeConstituent();
		D_Constituent mc_prev = getMeConstituent(previous);
		if(mecon!=mc_prev){
			if (DEBUG) System.out.println("GUISH: tellListeners: new mecon "+((mecon==null)?"null":mecon.getNameFull()));
	...
	public void orgUpdate(String orgID, int col, D_Organization org) {
		if((org==null) &&  (orgID!=null)) {
			org = D_Organization.getOrgByLID_NoKeep(orgID, true);
		}
		setSelectedOrg(org);
	}
	@Override
	...
	public void motion_update(String motID, int col, D_Motion mot) {
		if ((mot == null) &&  (motID != null)) {
			mot = D_Motion.getMotiByLID(motID, true, false);
		}
		setSelectedMot(mot);
	}
	@Override
	...
	public void constituentUpdate(D_Constituent c, boolean me, boolean selected) {
		if(DEBUG) System.out.println("GUIStatusHistory: setSelected="+((c==null)?"null":c.getNameFull()));
	...
	public boolean is_crt_org(D_Organization candidate) {
		for (GUIStatus h : history) {
			Object ob = h.selected_organization;
			D_Organization dob = (D_Organization) ob;
			if (dob == candidate) return true;
		}
		return false;
	}
	public boolean is_crt_const(D_Constituent candidate) {
		for (GUIStatus h : history) {
			Object ob = h.selected_constituent;
			D_Constituent dob = (D_Constituent) ob;
			if (dob == candidate) return true;
		}
		return false;
	}
	public void droppingOrg(D_Organization org) {
		if (DEBUG) System.out.println("GUIStatusHistory:droppingOrg: start");
		for (int i = 0; i < this.history.size(); i++) {
			GUIStatus h = history.get(i);
			if (h.selected_organization == org) {
				h.selected_organization = null;
				if (i != crt) {
					history.remove(i);
					i--;
					if (i < crt) crt--;
					if (DEBUG) System.out.println("GUIStatusHistory:droppingOrg: fire remove="+i+"/"+crt+"//"+this.history.size());
				} else {
					if (DEBUG) System.out.println("GUIStatusHistory:droppingOrg: fire crt="+crt);
					this.fireOrgListeners();
				}
			}
		}
		if (DEBUG) System.out.println("GUIStatusHistory:droppingOrg: done");
	}
	public void droppingConst(D_Constituent constit) {
		if (DEBUG) System.out.println("GUIStatusHistory: droppingConst: start");
		for (int i = 0; i < this.history.size(); i++) {
			GUIStatus h = history.get(i);
			if (h.selected_constituent == constit) {
				h.selected_constituent = null;
				if (i != crt) {
					history.remove(i);
					i--;
					if (i < crt) crt--;
					if (DEBUG) System.out.println("GUIStatusHistory: droppingConst: fire remove="+i+"/"+crt+"//"+this.history.size());
				} else {
					if (DEBUG) System.out.println("GUIStatusHistory: droppingConst: fire crt="+crt);
					this.fireConstituentSelectedListeners();
				}
			}
		}
		for (int i = 0; i < this.history.size(); i++) {
			GUIStatus h = history.get(i);
			if (h.myself_constituent == constit) {
				h.myself_constituent = null;
				if (i != crt) {
					history.remove(i);
					i--;
					if (i < crt) crt--;
		...
		if (DEBUG) System.out.println("GUIStatusHistory: droppingConst: done");
	}
}
class GUIStatus {
	...
	public String toString(int i) {
		...
		if(selected_motion!=null) r+=s(i)+"moti="+selected_motion.getMotionTitle().title_document.getDocumentUTFString()+"\n";
		if(selected_justification!=null) r+=s(i)+"just="+selected_justification.getJustificationTitle().title_document.getDocumentUTFString()+"\n";
