...
import net.ddp2p.common.data.SaverThreadsConstants;
	...
	public static class D_DirectoryEntry extends ASNObj implements DDP2P_DoubleLinkedList_Node_Payload
	{
		...
		public void buildInstanceRequestedTerms(){
			...
			String sql1 =
					...
					" WHERE "+net.ddp2p.common.table.subscriber.GID+" = ?" +
			...
			String sql111 =
					"SELECT "+net.ddp2p.common.table.subscriber.fields_subscribers+
					" FROM "+net.ddp2p.common.table.subscriber.TNAME+
					" WHERE "+net.ddp2p.common.table.subscriber.GID+" is NULL" +
					" AND   "+net.ddp2p.common.table.subscriber.GID_hash+" is NULL ;";
			String sql =
...
//					this.addresses = dec.getSequenceOf(Address.getASN1Type(), new Address[0], new Address());
		...
		long update() throws P2PDDSQLException{
			...
			params[net.ddp2p.common.table.registered.REG_INSTANCE] = instance;
			params[net.ddp2p.common.table.registered.REG_BRANCH] = branch;
...
//					new String[]{table.registered.global_peer_ID,table.registered.certificate,table.registered.addresses,table.registered.signature,table.registered.timestamp},
		...
		private void initGIDH(String _globalIDH) {
			...
			clean();
			String sql =
					"SELECT "+net.ddp2p.common.table.registered.fields+
					" FROM "+net.ddp2p.common.table.registered.TNAME+
		...
		public void initAll(ArrayList<ArrayList<Object>> d) {
			...
			for (ArrayList<Object> a : d) {
					Object _instance = a.get(net.ddp2p.common.table.registered.REG_INSTANCE); 
					if (_instance == null) {
						init(d.get(0), this);
						this.known = true;
						continue;
...
//			}
			...
			this.globalIDhash = Util.getString(d.get(net.ddp2p.common.table.registered.REG_GID_HASH));
			this.instance = Util.getString(d.get(net.ddp2p.common.table.registered.REG_INSTANCE));
			this.branch = Util.getString(d.get(net.ddp2p.common.table.registered.REG_BRANCH));
			this.agent_version = Util.getString(d.get(net.ddp2p.common.table.registered.REG_AGENT_VERSION));
			this.name = Util.getString(d.get(net.ddp2p.common.table.registered.REG_NAME));
			this.signature = Util.byteSignatureFromString(Util.getString(d.get(net.ddp2p.common.table.registered.REG_SIGN)));
			this.timestamp = Util.getCalendar(Util.getString(d.get(net.ddp2p.common.table.registered.REG_TIME)));
			this.known = true;
	...
	public static class D_Directory_Storage {
		 ...
		 *
		 */
		public static class SaverThread extends net.ddp2p.common.util.DDP2P_ServiceThread {
