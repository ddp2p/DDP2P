...
class D_Organization extends ASNObj implements  DDP2P_DoubleLinkedList_Node_Payload<D_Organization>, Summary { 
	...
	public D_OrgParams params = new D_OrgParams(); 
	public D_OrgConcepts concepts = new D_OrgConcepts(); 
	...
	public net.ddp2p.common.data.D_Message requested_data[]; 
	public ASNNeighborhoodOP neighborhoods[]; 
	public ASNConstituentOP constituents[]; 
	public D_Witness[] witnesses; 
	public D_Motion motions[]; 
	public D_Justification justifications[]; 
	public D_Vote signatures[]; 
	...
	public String _preapproved = null; 
		...
		private static void register_loaded(D_Organization crt) {
			...
			synchronized(loaded_objects) {
				...
				while ((loaded_objects.size() > SaverThreadsConstants.MAX_LOADED_ORGS)
					...
					if (loaded_objects.size() <= SaverThreadsConstants.MIN_LOADED_ORGS) break; 
					...
					D_Organization removed = loaded_objects.removeTail();
	...
	private void init(ArrayList<Object> row, boolean extra_fields, boolean extra_creator) throws P2PDDSQLException {
		...
		this.params.default_scoring_options = D_OrgConcepts.stringArrayFromString(Util.getString(row.get(net.ddp2p.common.table.organization.ORG_COL_SCORES),null));
		...
		this.params.icon = Util.byteSignatureFromString(i64); 
		...
		creator = D_Peer.getPeerByLID_NoKeep(creator_ID, true); 
		if (extra_creator) {
			if (creator == null) {
	...
	private static byte[] getSignatureFromString(String s) {
		...
		int pref_len = D_GIDH.d_OrgGrassSign.length(); 
	...
	public static String getOrgGIDHashGuess(String GID) {
		if (GID == null) return null;
		if (GID.startsWith(D_GIDH.d_OrgAuthSign)) return GID; 
		if (GID.startsWith(D_GIDH.d_OrgGrassSign)) return GID; 
	...
	public static String getOrgGIDGuess(String GID) {
		if (GID == null) return null;
		if (GID.startsWith(D_GIDH.d_OrgAuthSign)) return null; 
		if (GID.startsWith(D_GIDH.d_OrgGrassSign)) return GID; 
		return GID;
	}
	public static boolean isOrgGID(String GID) {
		if (GID == null) return false;
		if (GID.startsWith(D_GIDH.d_OrgAuthSign)) return false; 
		if (GID.startsWith(D_GIDH.d_OrgGrassSign)) return true; 
		return true;
	}
	public static boolean isOrgGIDH(String GID) {
		if (GID == null) return false;
		if (GID.startsWith(D_GIDH.d_OrgAuthSign)) return true; 
		if (GID.startsWith(D_GIDH.d_OrgGrassSign)) return true; 
		return false;
	}
	...
	private long _storeAct() throws P2PDDSQLException {
		...
		synchronized(D_Organization.lock_organization_GID_storage ) {
			...
			if (this.dirty_params || dirty_locals || dirty_main || this.dirty_preferences) {
				...
				p[net.ddp2p.common.table.organization.ORG_COL_NAME_JUST] = D_OrgConcepts.stringFromStringArray(concepts.name_justification);
				p[net.ddp2p.common.table.organization.ORG_COL_LANG] = D_OrgConcepts.stringFromStringArray(params.languages);
				...
				p[net.ddp2p.common.table.organization.ORG_COL_SCORES] = D_OrgConcepts.stringFromStringArray(params.default_scoring_options);
				...
				p[net.ddp2p.common.table.organization.ORG_COL_ARRIVAL] = getArrivalDate_str(); 
				p[net.ddp2p.common.table.organization.ORG_COL_RESET_DATE] = (reset_date!=null)?Encoder.getGeneralizedTime(reset_date):null; 
				...
				if (filter == 0) {
					setLID_AndLink(result = Application.getDB().insertNoSync(net.ddp2p.common.table.organization.TNAME, net.ddp2p.common.table.organization.fields_noID, p, DEBUG));
					...
					Application.getDB().updateNoSync(net.ddp2p.common.table.organization.TNAME,  net.ddp2p.common.table.organization.fields_noID, new String[]{net.ddp2p.common.table.organization.organization_ID}, p, DEBUG);
			...
			if (this.dirty_mydata) {
				...
				if (mydata.row <= 0) {
					mydata.row = Application.getDB().insertNoSync(net.ddp2p.common.table.my_organization_data.TNAME, net.ddp2p.common.table.my_organization_data.fields_noID, p, DEBUG);
	...
	public boolean isGrassroot() {
		...
		if (GIDH.startsWith(D_GIDH.d_OrgGrassSign)) return true; 
	...
	public boolean isAuthoritarian() {
		String GIDH = this.getGIDH_or_guess();
		if (GIDH == null) return false;
		if (GIDH.startsWith(D_GIDH.d_OrgGrassSign)) return false; 
	...
	public static long storeRemote(D_Organization oRG, boolean[] _changed,
		...
		if ( 
			...
			return  D_Organization.getLIDbyGID(oRG.getGID());
		}
 ...
 */
	...
	public String getFieldExtraID(String fieldGID, boolean insertTmp) throws P2PDDSQLException, ExtraFieldException {
		...
		String id = Util.getStringID(getFieldExtraID(fieldGID)); 
	...
	public boolean setIcon(byte[] icon) {
		...
		if (icon == null && this.params.icon == null) return true; 
	...
	public void __run() {
		synchronized(D_Organization_SaverThread.saver_thread_monitor) {
			...
			if (de != null) {
				...
				else D_Organization.need_saving_remove(de.getGIDH_or_guess());
