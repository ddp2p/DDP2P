...
public class D_Peer extends ASNObj implements DDP2P_DoubleLinkedList_Node_Payload<D_Peer>, Summary  {
	...
	public static class D_Peer_Preferences {
		...
		public String my_broadcastable_obj; 
		public String my_ID;
	...
	public static class D_Peer_Node {
		...
		private static void register_fully_loaded(D_Peer crt) {
			...
			synchronized(getLoaded_objects()) {
				crt.component_node.message = message; 
				if(crt.component_node.message != null) setCurrent_space(getCurrent_space() + crt.component_node.message.length);
			}
		}
		private static void unregister_loaded(D_Peer crt) {
			synchronized(getLoaded_objects()) {
				getLoaded_objects().remove(crt);
				getLoaded_By_LocalID().remove(new Long(crt.getLID()));
				getLoaded_By_GID().remove(crt.getGID());
				getLoaded_By_GIDhash().remove(crt.getGIDH_force());
			}
		}
		private static void register_loaded(D_Peer crt) {
			crt.reloadMessage();
			synchronized(getLoaded_objects()) {
				getLoaded_objects().offerFirst(crt);
				long lid = crt.getLID();
				if (lid > 0) {
					...
					if (old != null && old != crt) {
						...
						if (o_gidh != null) getLoaded_By_GIDhash().remove(o_gidh);
					}
					getLoaded_By_LocalID().put(oLID, crt);
				}
				String gidh = crt.getGIDH_force();
				String gid = gidh; 
				if (gid != null) { 
					D_Peer old = getLoaded_By_GID().get(gid);
					if (old != null && old != crt) {
						Util.printCallPath("D_Peer conflict: gid old="+old+" crt="+crt);
					}
					getLoaded_By_GID().put(gid, crt);
				}
				if (gidh != null) { 
					D_Peer old = getLoaded_By_GIDhash().get(gidh);
					if (old != null && old != crt) {
						Util.printCallPath("D_Peer conflict: gidh old="+old+" crt="+crt);
					}
					getLoaded_By_GIDhash().put(gidh, crt);
				}
				if (crt.component_node.message != null) setCurrent_space(getCurrent_space() + crt.component_node.message.length);
				int tries = 0;
				while((getLoaded_objects().size() > SaverThreadsConstants.MAX_LOADED_PEERS)
						|| (getCurrent_space() > SaverThreadsConstants.MAX_PEERS_RAM)) {
					if(getLoaded_objects().size() <= SaverThreadsConstants.MIN_LOADED_PEERS) break; 
					if (tries > MAX_TRIES) break;
					tries ++;
					D_Peer candidate = getLoaded_objects().getTail();
					...
					D_Peer removed = getLoaded_objects().removeTail();
					getLoaded_By_LocalID().remove(new Long(removed.getLID_keep_force()));
					getLoaded_By_GID().remove(removed.getGID());
					getLoaded_By_GIDhash().remove(removed.getGIDH_force());
					if (DEBUG) System.out.println("D_Peer: register_loaded: remove GIDH="+removed.getGIDH_force());
					if(removed.component_node.message != null) setCurrent_space(getCurrent_space() - removed.component_node.message.length);				
				}
			}
		}
		/**
		...
		private static void setRecent(D_Peer crt) {
			getLoaded_objects().moveToFront(crt);
		}
		/**
		...
		private static boolean register_newGID_ifLoaded(D_Peer crt) {
			...
			synchronized (getLoaded_objects()) {
				...
				Long oLID = new Long(lid);
				D_Peer _old = getLoaded_By_LocalID().get(oLID);
				if (_old == null) {
		...
		private static DDP2P_DoubleLinkedList<D_Peer> getLoaded_objects() {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			return Application.installations[crt_instalation].d_peer_loaded_objects;
		}
		private static void setLoaded_objects(DDP2P_DoubleLinkedList<D_Peer> loaded_objects) {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			Application.installations[crt_instalation].d_peer_loaded_objects = loaded_objects;
		}
		private static Hashtable<Long, D_Peer> getLoaded_By_LocalID() {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			return Application.installations[crt_instalation].d_peer_loaded_By_LocalID;
		}
		private static void setLoaded_By_LocalID(Hashtable<Long, D_Peer> loaded_By_LocalID) {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			Application.installations[crt_instalation].d_peer_loaded_By_LocalID = loaded_By_LocalID;
		}
		private static Hashtable<String, D_Peer> getLoaded_By_GID() {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			return Application.installations[crt_instalation].d_peer_loaded_By_GID;
		}
		private static void setLoaded_By_GID(Hashtable<String, D_Peer> loaded_By_GID) {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			Application.installations[crt_instalation].d_peer_loaded_By_GID = loaded_By_GID;
		}
		private static Hashtable<String, D_Peer> getLoaded_By_GIDhash() {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			return Application.installations[crt_instalation].d_peer_loaded_By_GIDhash;
		}
		private static void setLoaded_By_GIDhash(Hashtable<String, D_Peer> loaded_By_GIDhash) {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			Application.installations[crt_instalation].d_peer_loaded_By_GIDhash = loaded_By_GIDhash;
		}
		private static long getCurrent_space() {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			return Application.installations[crt_instalation].d_peer_current_space;
		}
		private static void setCurrent_space(long current_space) {
			int crt_instalation = Application.getCurrentInstallationFromThread();
			Application.installations[crt_instalation].d_peer_current_space = current_space;
		}
	}	
	boolean temporary = false; 
	boolean loaded_basics = false; 
	boolean loaded_locals = false; 
	boolean loaded_globals = false; 
	boolean loaded_addresses = false; 
	boolean loaded_served_orgs = false; 
	boolean loaded_instances = false; 
	boolean loaded_my_data = false; 
	boolean loaded_local_preferences = false; 
	boolean loaded_local_agent_state = false; 
	/**
	...
	@Deprecated
	public String[] signature_alg = net.ddp2p.common.hds.SR.HASH_ALG_V1; 
	...
	public D_PeerOrgs[] served_orgs = null; 
	...
	public void init_ByGID(String gID, String peerGIDhash, boolean create, D_Peer __peer) throws P2PDDSQLException{
		...
		if (p.size() == 0) {
			...
			this.component_basic_data.globalIDhash = peerGIDhash; 
			if (__peer != null) {
	...
	private void _init(ArrayList<Object> p, boolean encode_addresses, boolean encode_served_orgs) {
		...
		component_basic_data.globalID = Util.getString(p.get(net.ddp2p.common.table.peer.PEER_COL_GID)); 
		component_basic_data.globalIDhash = Util.getString(p.get(net.ddp2p.common.table.peer.PEER_COL_GID_HASH)); 
		component_basic_data.name =  Util.getString(p.get(net.ddp2p.common.table.peer.PEER_COL_NAME)); 
		...
		component_basic_data.slogan = Util.getString(p.get(net.ddp2p.common.table.peer.PEER_COL_SLOGAN)); 
		...
		if (encode_served_orgs) {
			served_orgs = _getPeerOrgs(getLID()); 
			loaded_globals = true;
		...
		if (DEBUG) System.out.println("D_Peer: _init: "+getLID()+" : "+Util.concat(served_org_inferred, ",", "NULL"));
	...
	private final static String sql_peer_org_inf = 
				...
				+ " WHERE " + net.ddp2p.common.table.peer_org_inferred.peer_ID + " = ?;";
	/**
	...
	private ArrayList<D_PeerOrgInferred> __getPeerOrgInferred(long pLID) {
		...
		if (pLID <= 0) {
			if (DEBUG) System.out.println("D_Peer: __getPeerOrgInferred: return empty no peer");
			return new ArrayList<D_PeerOrgInferred>();
		}
		try {
			ArrayList<ArrayList<Object>> poi = 
					Application.getDB().select(sql_peer_org_inf,
							new String[]{Util.getStringID(pLID)}, DEBUG);
			ArrayList<D_PeerOrgInferred> result = new ArrayList<D_PeerOrgInferred>();
			for (int i = 0 ; i < poi.size() ; i ++ ) {
				if (DEBUG) System.out.println("D_Peer: __getPeerOrgInferred: add poi "+i);
				ArrayList<Object> ipoi = poi.get(i);
				result.add( new D_PeerOrgInferred(ipoi) );
			}
			if (DEBUG) System.out.println("D_Peer: __getPeerOrgInferred: return");
			return result;
		} catch (P2PDDSQLException e) {
			e.printStackTrace();
		}
		if (DEBUG) System.out.println("D_Peer: __getPeerOrgInferred: return empty");
		return new ArrayList<D_PeerOrgInferred>();
	}
	/**
	...
	final static String queryOrgs = "SELECT o."+
	...
	static private D_Peer getPeerByLID_AttemptCacheOnly_NoKeep(long ID, boolean load_Globals) {
		if (ID <= 0) return null;
		Long id = new Long(ID);
		D_Peer crt = D_Peer_Node.getLoaded_By_LocalID().get(id);
	...
	static public D_Peer getPeerByPeer_Keep(D_Peer peer) {
		...
		if (result == null)
			result = getPeerByLID(peer.getLID(), true, true); 
		if (result == null) {
	...
	public void storeRequest() {
		...
		try {
			if (!getSaverThread().isAlive()) { 
				if (DEBUG) System.out.println("D_Peer:storeRequest:startThread");
				getSaverThread().start();
			}
			synchronized(getSaverThread()) {getSaverThread().notify();}
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void storeTesterRecommendation() {
		...
		params[net.ddp2p.common.table.peer_scheduled_message.F_CREATION_DATE] =  Util.getGeneralizedTime();
		try {
			if (update) {
	...
	private void storeBasicComponent() throws P2PDDSQLException {
		...
		synchronized (dirty_main_monitor) {
			...
			params[net.ddp2p.common.table.peer.PEER_COL_SIGN]=getSignatureDB(); 
			params[net.ddp2p.common.table.peer.PEER_COL_CREATION]=this.getCreationDate(); 
		...
		if (this.served_org_inferred_orig != null) {
			for (D_PeerOrgInferred p_org_inf_orig : this.served_org_inferred_orig) {
				...
				if (pi == null) {
					...
					if (pi.peer_org_inferred_ID <= 0) pi.peer_org_inferred_ID = p_org_inf_orig.peer_org_inferred_ID;
	...
	public void makeNewInstance() {
		...
		do {
			...
			this.instance = Encoder.getGeneralizedTime(cal);
	...
	public D_Peer decode(Decoder dec) throws ASN1DecoderFail {
		...
		else this.setCreationDateNoDirty(null, null);
	...
	public Encoder getSignatureEncoder_V0(){
		...
		enc.addToSequence(new Encoder(new byte[0])); 
		enc.setASN1Type(TAG);
		return enc;
	}
	...
	addAddress(Address _address, Calendar _arrival_date,
			String arrival_date, 
			boolean keep_old_priority_if_exists, D_PeerInstance dpi) 
			{
	...
	public byte[] sign(net.ddp2p.ciphersuits.SK sk){
		...
		else this.last_signature_verified_successful = true; 
	...
	static D_Peer need_saving_next() {
		...
		D_Peer r = D_Peer_Node.getLoaded_By_GIDhash().get(c);
		if (r == null) {
			if (_DEBUG) {
	...
	public static String dumpDirCache() {
		String s = "[";
		s += D_Peer_Node.getLoaded_objects().toString();
		s += "]";
		return s;
	}
	...
	public boolean loadRemote(D_Peer pa, RequestData sol_rq, RequestData new_rq) {
		...
		if(this.component_basic_data.picture == null)
			this.component_basic_data.picture = pa.component_basic_data.picture; 
	...
	public static void reset(String peerID, Calendar reset) {
		...
		if (p._instances != null) {
			for (D_PeerInstance pi : p._instances.values()) { 
				if (pi != null) {
	...
	public void setServingOrgInferred(long orgID) {
		...
		if (DEBUG) System.out.println("D_Peer: setServingOrgInferred: new poi = "+poi);
		synchronized(this.served_org_inferred) {
		 ...
		 * TODO
		 */
		if (DEBUG) System.out.println("D_Peer: setServingOrgInferred = adding "+orgID+" to "+this);
		this.storeSynchronouslyNoException();
	}
	...
	public void purge() {
    	...
    	try {
			...
			Application.getDB().deleteNoSync(net.ddp2p.common.table.peer_instance.TNAME, new String[]{net.ddp2p.common.table.peer.peer_ID}, new String[]{peerID}, DEBUG); 
			...
			Application.getDB().sync(new ArrayList<String>(Arrays.asList(net.ddp2p.common.table.peer.TNAME, net.ddp2p.common.table.peer_my_data.TNAME, net.ddp2p.common.table.org_distribution.TNAME,
					net.ddp2p.common.table.peer_org.TNAME,net.ddp2p.common.table.peer_plugin.TNAME, net.ddp2p.common.table.peer.TNAME))); 
	...
	static public boolean delete(D_Peer peer) throws P2PDDSQLException {
		synchronized(monitor_object_factory) {
			...
			Application.getDB().deleteNoSync(net.ddp2p.common.table.peer.TNAME, new String[]{net.ddp2p.common.table.peer.peer_ID}, new String[]{pLID}, DEBUG); 
	...
	public static D_Peer_SaverThread getSaverThread() {
		return Application.get_D_Peer__SaverThread();
	}
	public static void setSaverThread(D_Peer_SaverThread saverThread) {
		Application.set_D_Peer__SaverThread(saverThread);
	}
}
