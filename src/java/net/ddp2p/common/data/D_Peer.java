...
//				Util.printCallPath("D_Peer: register_loaded: enter\n"+crt);
//				for (String s : getLoaded_By_GID().keySet())  System.out.println("D_Peer: register_loaded: dump gid ="+s);
//				for (String s : getLoaded_By_GIDhash().keySet())  System.out.println("D_Peer: register_loaded: dump gidh="+s);
//				for (Long s : getLoaded_By_LocalID().keySet())  System.out.println("D_Peer: register_loaded: dump id  ="+s);
//				System.out.println("D_Peer: register_loaded: dump:\n"+ getLoaded_objects().toString()+"\nDump Done");
				...
				while((getLoaded_objects().size() > SaverThreadsConstants.MAX_LOADED_PEERS)
					...
					if(getLoaded_objects().size() <= SaverThreadsConstants.MIN_LOADED_PEERS) break; // at least _crt_peer and _myself
					...
					D_Peer removed = getLoaded_objects().removeTail();//remove(loaded_peers.size()-1);
		...
		public static boolean register_newLID_ifLoaded(D_Peer crt) {
			...
			synchronized (getLoaded_objects()) {
				...
				D_Peer old = getLoaded_By_GIDhash().get(gidh); //getByGIDH(gidh, );
				if (old == null) {
...
//	Factories from GID
	...
	static private D_Peer getPeerByGID_or_GIDhash_AttemptCacheOnly_NoKeep(String GID, String GIDhash, boolean load_Globals) {
		...
		//if ((GIDhash != null) && ! D_Peer.isGIDHash(GIDhash)) GIDhash = D_Peer.getGIDHashGuess(GIDhash);
...
//		p = D_Peer.getPeerByPeer_Keep(p);
	...
	public void setServingOrgInferred(long orgID, boolean serve) {
		assertReferenced();
//		if (! this.loaded_served_orgs)
//			this.loadServedOrgsLocals();
	...
	public void setServingOrgInferred(long orgID) {
		...
		/*
		for (D_PeerOrgInferred pois : this.served_org_inferred_orig) {
			if (_DEBUG) System.out.println("D_Peer: setServingOrgInferred: orig poi = "+pois);
		}
		for (D_PeerOrgInferred pois : this.served_org_inferred) {
			if (_DEBUG) System.out.println("D_Peer: setServingOrgInferred: old poi = "+pois);
		}
		*/
