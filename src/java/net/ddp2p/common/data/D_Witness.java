...
class D_Witness extends ASNObj implements Summary {
	...
	public String global_witness_ID; 
	...
	public String witness_eligibility_category; 
	public int sense_y_n; 
	public String witness_trustworthiness_category = null; 
	...
	static String sqlWNLID = 
			...
			+ Util.setDatabaseAlias(net.ddp2p.common.table.witness.witness_fields, "w")+" "
			+" FROM "+net.ddp2p.common.table.witness.TNAME+" AS w "+
			" WHERE w."+net.ddp2p.common.table.witness.source_ID+"=? AND w."+net.ddp2p.common.table.witness.neighborhood_ID+"=?;"
			;
	/**
	...
	public static D_Witness getWitnessForNeighborhoodLID(long _witnessLID, long _targetNeighLID) {
		...
		if (_witnessLID <= 0 || _targetNeighLID <= 0) return null; 
	...
	static String sqlAWNLID = 
			...
			+ Util.setDatabaseAlias(net.ddp2p.common.table.witness.witness_fields, "w")+" "
			+" FROM "+net.ddp2p.common.table.witness.TNAME+" AS w "+
			" WHERE w."+net.ddp2p.common.table.witness.neighborhood_ID+"=?;"
			;
	/**
	...
	public static D_Witness[] getAllWitnessForNeighborhoodLID(long _targetNeigLID, int limit) {
		...
		if (_targetNeigLID <= 0) return new D_Witness[0];
		ArrayList<ArrayList<Object>> w = null;
		D_Witness[] result; 
		try {
	...
	static String sqlWCLID = 
			...
			+ Util.setDatabaseAlias(net.ddp2p.common.table.witness.witness_fields, "w")+" "
			+" FROM "+net.ddp2p.common.table.witness.TNAME+" AS w "+
			" WHERE w."+net.ddp2p.common.table.witness.source_ID+"=? AND w."+net.ddp2p.common.table.witness.target_ID+"=?;"
			;
	/**
	...
	public static D_Witness getWitnessForConstituentLID(long _witnessLID, long _targetConsLID) {
		...
		if (_witnessLID <= 0 || _targetConsLID <= 0) return null; 
		ArrayList<ArrayList<Object>> w = null;
		D_Witness result; 
		try {
	...
	public static D_Witness[] getAllWitnessForConstituentLID(long _targetConsLID, int limit) {
		...
		if (_targetConsLID <= 0) return new D_Witness[0];
		ArrayList<ArrayList<Object>> w = null;
		D_Witness[] result; 
		try {
	...
	static String sqlSWCLID = 
			...
			+ Util.setDatabaseAlias(net.ddp2p.common.table.witness.witness_fields, "w")+" "
			+" FROM "+net.ddp2p.common.table.witness.TNAME+" AS w "+
			" WHERE w." + net.ddp2p.common.table.witness.sense_y_n + ">=? AND w." + net.ddp2p.common.table.witness.target_ID + "=? LIMIT 1;"
			;
	/**
	...
	public static D_Witness getSupportWitnessForConstituentLID(String supportString, long _targetConsLID) {
		...
		if (supportString == null || _targetConsLID <= 0) return null; 
		ArrayList<ArrayList<Object>> w = null;
		D_Witness result; 
		try {
	...
	static String sqlSWNLID = 
			...
			+ Util.setDatabaseAlias(net.ddp2p.common.table.witness.witness_fields, "w")+" "
			+" FROM "+net.ddp2p.common.table.witness.TNAME+" AS w "+
			" WHERE w." + net.ddp2p.common.table.witness.sense_y_n + ">=? AND w." + net.ddp2p.common.table.witness.neighborhood_ID + "=? LIMIT 1;"
			;
	/**
	 * Get a witness with support at equal or greater to the integer value in "supportString" (>="1")
	 * @param supportString
	...
	public static D_Witness getSupportWitnessForNeigborhoodLID(String supportString, long _targetNeigLID) {
		...
		if (supportString == null || _targetNeigLID <= 0) return null; 
		ArrayList<ArrayList<Object>> w = null;
		D_Witness result; 
		try {
	...
	public void init(ArrayList<Object> w) {
		...
		hash_alg = Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_HASH_ALG));
		global_witness_ID = Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_GID));
		creation_date = Util.getCalendar(Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_CREATION_DATE)));
		arrival_date = Util.getCalendar(Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_ARRIVAL_DATE)));
		witness_eligibility_category = Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_CAT));
		sense_y_n = Util.ival(w.get(net.ddp2p.common.table.witness.WIT_COL_SENSE),UNKNOWN);
		sense_y_trustworthiness = Util.ival(w.get(net.ddp2p.common.table.witness.WIT_COL_SENSE_TRUSTWORTHINESS),UNKNOWN);
		this.witness_trustworthiness_category = Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_CAT_TRUSTWORTHINESS));
		byte[] _statements = Util.byteSignatureFromString(Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_STATEMENTS)));
		if(_statements != null) statements = new D_WitnessStatements(new Decoder(_statements));
		signature = Util.byteSignatureFromString(Util.getString(w.get(net.ddp2p.common.table.witness.WIT_COL_SIGN)));
		if(DEBUG)System.out.println("D_Witness:init: sign="+Util.byteToHexDump(signature, 10)+"  from sign="+w.get(net.ddp2p.common.table.witness.WIT_COL_SIGN));
		witnessed_neighborhoodID = Util.lval(w.get(net.ddp2p.common.table.witness.WIT_COL_T_N_ID), -1);
		witnessed_constituentID = Util.lval(w.get(net.ddp2p.common.table.witness.WIT_COL_T_C_ID), -1);
		witnessing_constituentID = Util.lval(w.get(net.ddp2p.common.table.witness.WIT_COL_S_C_ID), -1);
		witnessID = Util.lval(w.get(net.ddp2p.common.table.witness.WIT_COL_ID), -1);
		witnessed_global_neighborhoodID = D_Neighborhood.getGIDFromLID(witnessed_neighborhoodID); 
		witnessed_global_constituentID = D_Constituent.getGIDFromLID(witnessed_constituentID); 
		...
		if (witness != null) {
			witnessing_global_constituentID = witness.getGID(); 
			global_organization_ID = witness.getOrgGID(); 
			organization_ID = witness.getOrganizationLIDstr(); 
		}
	...
	public String make_ID(){
		...
		return net.ddp2p.common.data.D_GIDH.d_Witn+Util.getGID_as_Hash(this.getHashEncoder().getBytes());
	}
	public boolean verifySignature(){
		...
		String pk_ID = this.witnessing_global_constituentID;
	...
	private long _monitored_storeVerified(boolean sync, Calendar arrival_date) throws P2PDDSQLException {
		...
		if (witnessed_neighborhoodID <= 0) {
			...
			" FROM "+net.ddp2p.common.table.witness.TNAME+
			" WHERE "+net.ddp2p.common.table.witness.source_ID+"=? AND "+net.ddp2p.common.table.witness.target_ID+"=?;";
		...
		if (witnessID <= 0) { 
			...
			if ((old_date == null) || _creation_date.compareTo(old_date) > 0) {
				...
				Application.getDB().update(sync, net.ddp2p.common.table.witness.TNAME, 
					...
					net.ddp2p.common.table.witness.creation_date,
					net.ddp2p.common.table.witness.arrival_date
					}, new String[]{net.ddp2p.common.table.witness.witness_ID},
					new String[]{
					global_witness_ID,
					hash_alg,
					Util.stringSignatureFromByte(signature),
	...
	public static ArrayList<String> checkAvailability(ArrayList<String> witn,
		...
		if (DEBUG) System.out.println("D_Witness: checkAvailability: enter");
		ArrayList<String> result = new ArrayList<String>();
		for (String hash : witn) {
			if (DEBUG) System.out.println("D_Witness: checkAvailability: consider "+hash);
			if (! available(hash, orgID, DBG)) {
				String wGIDHash = D_Witness.getGIDfromGID(hash);
				if (DEBUG) System.out.println("D_Witness: checkAvailability: GIDH = "+wGIDHash);
				if (wGIDHash != null) {
					result.add(wGIDHash);
				} else {
					if (_DEBUG) System.out.println("D_Witness: checkAvailability: null GIDH for "+hash);
				}
			} else {
				if (DEBUG) System.out.println("D_Witness: checkAvailability: available "+hash);
			}
		}
		return result;
	}
	...
	public static int isGIDavailable(String gID, boolean DBG) throws P2PDDSQLException {
		int r;
		String sql = 
			"SELECT "+net.ddp2p.common.table.witness.witness_ID+","+net.ddp2p.common.table.witness.signature+
			" FROM "+net.ddp2p.common.table.witness.TNAME+
			" WHERE "+net.ddp2p.common.table.witness.global_witness_ID+"=? ;";
    ...
    public static void witness(D_Constituent c){
    	...
    	String gcd = c.getGID();
     	try {
    		D_Constituent myself = Application_GUI.getMeConstituent(); 
