...
import net.ddp2p.common.data.D_Constituent.D_Constituent_Node;
import net.ddp2p.common.hds.ASNPoint;
...
class D_Neighborhood extends ASNObj implements   DDP2P_DoubleLinkedList_Node_Payload<D_Neighborhood>, Summary {
	...
	static final String sql_neighborhood = "SELECT  "+Util.setDatabaseAlias(net.ddp2p.common.table.neighborhood.fields_neighborhoods, "n")
		+ ", p."+net.ddp2p.common.table.neighborhood.global_neighborhood_ID
	...
	private void init(ArrayList<Object> N) {
		...
		setLIDstr(Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_ID)));
		description = (Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_DESCRIPTION)));
		name = (Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_NAME)));
		...
		String subs = Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_NAMES_DUBDIVISIONS));
		if (subs!=null) names_subdivisions = (splitSubDivisions(subs));
		picture = (Util.byteSignatureFromString(Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_PICTURE))));
		boundary = (null);
		signature = (Util.byteSignatureFromString(Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_SIGNATURE))));	
		arrival_date = (Util.getCalendar(_arrival_date = Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_ARRIVAL_DATE))));
		preferences_date = (Util.getCalendar(_preferences_date = Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_PREFERENCES_DATE))));
		blocked = (Util.stringInt2bool(Util.getString(N.get(net.ddp2p.common.table.neighborhood.IDX_BLOCKED)), false));	
	...
	private void init_my() {
		...
		String sql_my_neighborhood = 
				"SELECT "+ net.ddp2p.common.table.my_neighborhood_data.fields_list
		...
		try {
			...
			this.mydata.category = Util.getString(my.get(0).get(net.ddp2p.common.table.my_neighborhood_data.COL_CATEGORY));
			this.mydata.name = Util.getString(my.get(0).get(net.ddp2p.common.table.my_neighborhood_data.COL_NAME));
			this.mydata.row = Util.lval(my.get(0).get(net.ddp2p.common.table.my_neighborhood_data.COL_ROW));
	...
	public static long getNeighLIDByNameAndParent(String name, long parent_nID, long organizationID) {
		...
		try {
			...
			if (sel.size() != 0)
				return Util.lval(sel.get(0).get(net.ddp2p.common.table.neighborhood.IDX_ID));
		} catch(Exception e) {
			e.printStackTrace();
		}
		return -1;
	...
	private void storeAct_my() {
		...
		try {
			if (this.mydata.row <= 0) {
				param = new String[net.ddp2p.common.table.my_neighborhood_data.FIELDS_NB_NOID];
			} else {
				param = new String[net.ddp2p.common.table.my_neighborhood_data.FIELDS_NB];
			}
	...
	public long storeAct_main() throws P2PDDSQLException {
		...
		String id = this.getLIDstr();
	   	...
	   	this.dirty_local = this.dirty_main = this.dirty_preferences = false;
			...
			params[net.ddp2p.common.table.neighborhood.IDX_PREFERENCES_DATE] = this.getPreferencesDateStr();
			params[net.ddp2p.common.table.neighborhood.IDX_GID] = this.getGID();
			params[net.ddp2p.common.table.neighborhood.IDX_DESCRIPTION] = this.getDescription();
			params[net.ddp2p.common.table.neighborhood.IDX_SIGNATURE] = Util.stringSignatureFromByte(this.getSignature());
			params[net.ddp2p.common.table.neighborhood.IDX_ORG_ID] = this.getOrgLIDstr();
			params[net.ddp2p.common.table.neighborhood.IDX_NAME] = this.getName();
			...
			params[net.ddp2p.common.table.neighborhood.IDX_PEER_SOURCE_ID] = this.getPeerSourceLIDstr();
			if (id == null) {
	...
	public static String concatSubdivisions(String[] names_subdivisions) {
		...
		return ":"+Util.concat(names_subdivisions,net.ddp2p.common.table.neighborhood.SEP_names_subdivisions,null)+":";
	}
	/**
	...
	public static String[] splitSubDivisionsFromCAND(String subs) {
		return subs.split(Pattern.quote(net.ddp2p.common.table.neighborhood.SEP_names_subdivisions));
	}
	/**
	...
	public static String[] splitSubDivisions(String subs) {
		...
		String[] val = subs.split(Pattern.quote(net.ddp2p.common.table.neighborhood.SEP_names_subdivisions));
	...
	public static String getChildSubDivision(String names_subdivisions, int idx) {
		...
		if (names_subdivisions == null) return null;
		String [] splits = names_subdivisions.split(net.ddp2p.common.table.neighborhood.SEP_names_subdivisions,idx+3);
		if (splits.length < idx + 3) return null;
		return net.ddp2p.common.table.neighborhood.SEP_names_subdivisions+splits[idx+2];
	}
	/**
	...
	public static String _getChildSubDivision(String names_subdivisions, String crt) {
		boolean DEBUG = false;
		int k;
		String result=net.ddp2p.common.table.neighborhood.SEP_names_subdivisions;
		if(DEBUG) System.out.println("Util:getChildSubDivision:Child Subdivisions: "+names_subdivisions);
		if (names_subdivisions==null) return null;
		String[]splits = names_subdivisions.split(Pattern.quote(net.ddp2p.common.table.neighborhood.SEP_names_subdivisions));
		for(k=1; k<splits.length; k++)
			if (splits[k].equals(crt)) break;
		if(k==splits.length) return names_subdivisions;
		for(k++; k<splits.length; k++)
			if(!"".equals(splits[k])) result = result+splits[k]+net.ddp2p.common.table.neighborhood.SEP_names_subdivisions;
		return result;
	}
	/**
	 * Break subdivisions and reassemble after extracting out the prefix ending with crt
	...
	public static String[] _getChildSubDivisions(String names_subdivisions, String crt) {
		...
		String[] splits = result_c.split(Pattern.quote(net.ddp2p.common.table.neighborhood.SEP_names_subdivisions));
	...
	final public static String sql_n_children =
				"SELECT " + net.ddp2p.common.table.neighborhood.neighborhood_ID
				+ " FROM " + net.ddp2p.common.table.neighborhood.TNAME
				+ " WHERE " + net.ddp2p.common.table.neighborhood.parent_nID + "=? "
				+ " ORDER BY "+net.ddp2p.common.table.neighborhood.name+" DESC"
						+ ";";
	...
	public static ArrayList<String> checkAvailability(ArrayList<String> cons,
		...
		for (String hash : cons) {
			if (! available(hash, orgID, DBG)) {
				String nGIDHash = D_Neighborhood.getGIDfromGID(hash);
				if (nGIDHash != null)
					result.add(nGIDHash);
			}
		}
	...
	public static final String sql_all_root_neighborhoods =
			"SELECT " + net.ddp2p.common.table.neighborhood.neighborhood_ID +
			" FROM " + net.ddp2p.common.table.neighborhood.TNAME +
			" WHERE " + net.ddp2p.common.table.neighborhood.organization_ID +"=?" +
					" AND ( " + net.ddp2p.common.table.neighborhood.parent_nID + " IS NULL OR " +
					net.ddp2p.common.table.neighborhood.parent_nID + " < '1' ) ;";
	public static final String sql_all_neighborhoods_in_neigh =
			"SELECT " + net.ddp2p.common.table.neighborhood.neighborhood_ID + 
			" FROM " + net.ddp2p.common.table.neighborhood.TNAME +
			" WHERE " + net.ddp2p.common.table.neighborhood.organization_ID +"=? " +
					" AND " + net.ddp2p.common.table.neighborhood.parent_nID + "=?;";
...
class D_Neighborhood_SaverThread extends net.ddp2p.common.util.DDP2P_ServiceThread {
	...
	public void _run() {
		for (;;) {
			if (stop) return;
			if (net.ddp2p.common.data.SaverThreadsConstants.getNumberRunningSaverThreads() < SaverThreadsConstants.MAX_NUMBER_CONCURRENT_SAVING_THREADS && D_Neighborhood.getNumberItemsNeedSaving() > 0)
