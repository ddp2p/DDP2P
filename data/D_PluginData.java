...
import plugin_data.PeerConnection;
...
import ASN1.Decoder;
import ASN1.Encoder;
import table.plugin_local_storage;
import util.Util;
...
public class D_PluginData extends ASNObj implements PeerConnection{
	...
	public static void  distributeToPlugins(ArrayList<Msg> msgs_queue, String peer_GID) throws ASN1DecoderFail{
		...
		for(Msg m : msgs_queue) {
			...
			PluginMethods plugin = plugin_data.PluginRegistration.s_methods.get(id);
			if(plugin==null){
	...
	public static boolean _enqueueForSending(byte[] msg, String peerGID, String plugin_GID) {
		...
		try {
			synchronized(sync) {
				peerID = data.D_PeerAddress.getLocalPeerIDforGID(peerGID);
	...
	public static boolean _storeData(String key, byte[] data, String pluginGID) {
		try {
			...
			if(data == null){
				Application.db.delete(table.plugin_local_storage.TNAME,
						new String[]{table.plugin_local_storage.plugin_ID, table.plugin_local_storage.plugin_key},
						new String[]{ID, key},
						DEBUG);
			}
			...
			String sql=
				...
				" WHERE "+table.plugin_local_storage.plugin_ID+"=? AND "+table.plugin_local_storage.plugin_key+"=?;";
			ArrayList<ArrayList<Object>> d = Application.db.select(sql, new String[]{ID, key}, DEBUG);
			if(d.size()!=0) {
				Application.db.update(table.plugin_local_storage.TNAME,
						new String[]{table.plugin_local_storage.data}, 
						new String[]{table.plugin_local_storage.plugin_ID,table.plugin_local_storage.plugin_key}, 
						new String[]{_data, ID, key}, DEBUG);
				
				return true;
			}else{
				Application.db.insert(table.plugin_local_storage.TNAME,
