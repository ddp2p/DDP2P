...
public class DD {
	...
	public static final String VERSION = "0.9.55";
	...
	*/
	...
	public static boolean test_proper_directory(String ld) {
    	...
    	for(int k=0; k<dirs.length; k++) {
    		if(dirs[k] == null){
    			Application_GUI.warning(_("Test Error for "+dirs[k]), _("Error installing directories (null)"));
    			return false;
    		}
    		...
    		//if(d.length!=2){
    			Application_GUI.warning(_("Error for")+" "+dirs[k]+"\nParsing Error: "+e.getMessage(), _("Error installing directories (impropper)"));
    			return false;
    		}
    		...
    		try{
    			...
    			Application_GUI.warning(_("Error for")+" "+dirs[k]+"\nConnection Error: "+e.getMessage(), _("Error installing directories"));
    			return false;
    		}
    	}
		return true;
		...
		if ((dirs == null) || (dirs.length == 0)) {
     	...
     	if ((dirs == null) || (dirs.length == 0)) {
    		if (! DD.WARNED_NO_DIRS) {
    			Application_GUI.warning(_("Currently there are no listing_directories for connections found at Connections initialization: " +
    	...
    	*/
    	...
    	for(int k=0; k<dirs.length; k++) {
    		...
    		try{
    			...
    			Application_GUI.warning(_("Error for")+" "+dirs[k]+"\nLoad Error: "+e.getMessage(), _("Error installing directories"));
    		}
    	}
	}
			...
			if(rows.size()>0){
				...
				Application.db.updateNoSync(
					...
					try{
						...
						e.printStackTrace();
						Application_GUI.warning(_("Error inserting:")+"\n"+_("value=")+Util.trimmed(value)+"\n"+_("field=")+field+"\n"+_("Error:")+e.getLocalizedMessage(), _("Database update error"));
					}
					if(DEBUG){
						Application_GUI.warning(_("Added absent property: ")+field, _("Properties"));
    	...
    	if (value!=null){
    		...
    		if (!value.equals(old_val)) {
    			db.insert(
    					...
    					DEBUG);
    			if(DEBUG)Application_GUI.warning(_("Added absent property: ")+field, _("Properties"));
    		}
    	}
