...
public class MainFrame {
	...
	static String TAB_DIRS = _("Address Books");
	static int TAB_DIRS_ = 5;
	
	static String TAB_PEERS = _("Safes");
	...
	static String TAB_CONN = _("Connections");
	static int TAB_CONN_ = 13;
	static final JPanel JunkPanelCONN = new JPanel();
	
	static String TAB_CLONE = _("Clones");
	    ...
	    public void stateChanged(ChangeEvent e) {
	    	...
	    	*/
	    	...
	    	if(MainFrame.TAB_MOTS_ == index) {
				if(DD.DEBUG) System.out.println("DD.tabsChanged: motions index");
	    		if(!_mots) {
	    			if(_saved_mots==null) {
		    			_saved_mots = new widgets.motions.Motions();
		    			MainFrame.tabbedPane.setComponentAt(index, _saved_mots.getComboPanel());
						if(DD.DEBUG) System.out.println("DD.tabsChanged: motions idx component set");
	    			}
		    		if(_mots_reload || _mots_disconnect) {
						if(DD.DEBUG) System.out.println("DD.tabsChanged: motions idx will connect");
		    			_saved_mots.connectWidget(); // after getPanel()
						if(DD.DEBUG) System.out.println("DD.tabsChanged: motions idx connected");
		    		}
	    			_mots = true;
	    		}
	    	}else{
	...
	public static Component makeMotionPanel( Component _medit, Component motions) {
		...
		if(_medit instanceof MotionsListener) {
			...
			if(DD.DEBUG) System.out.println("DD.makeMotionPanel: return splitter");
			return  result;
		}else{
			if(DD.DEBUG) System.out.println("DD.makeMotionPanel: return motion_scroll");
	...
	public static void createAndShowGUI() throws P2PDDSQLException{
	    	...
	    	if(DD.preloadedControl) {
	    		...
	    		MainFrame.tabbedPane.addTab(MainFrame.TAB_SETTINGS, icon_conf, MainFrame.controlPane, _("Configuration"));
	    	}else
	    		MainFrame.tabbedPane.addTab(MainFrame.TAB_SETTINGS, icon_conf, MainFrame.JunkControlPane, _("Configuration"));
	        
	        MainFrame.tabbedPane.setMnemonicAt(MainFrame.TAB_SETTINGS_, KeyEvent.VK_S);
	        //tabbedPane.addTab(_("Keys"), keys.getPanel());
	        MainFrame.tabbedPane.addTab(MainFrame.TAB_KEYS, icon_key, MainFrame.JunkPanelKeys, _("Keys"));
	        MainFrame.tabbedPane.setMnemonicAt(MainFrame.TAB_KEYS_, KeyEvent.VK_K);
	        MainFrame.tabbedPane.addTab(MainFrame.TAB_ID, icon_identity, MainFrame.identitiesPane, _("Identities"));
	        MainFrame.tabbedPane.setMnemonicAt(MainFrame.TAB_ID_, KeyEvent.VK_I);
	        MainFrame.tabbedPane.addTab(MainFrame.TAB_CONS, icon_con, MainFrame.constituentsPane, _("Constituents"));
	        MainFrame.tabbedPane.setMnemonicAt(MainFrame.TAB_CONS_, KeyEvent.VK_C);
	        if(MainFrame.SONG)MainFrame.tabbedPane.addTab(_("Census"), icon_census, _censusPane, _("Census"));
	        MainFrame.tabbedPane.addTab(MainFrame.TAB_DIRS, icon_dir, listing_dicts.getPanel(), _("Directory"));
			...
			MainFrame.tabbedPane.addTab(MainFrame.TAB_PEERS, icon_peer, MainFrame.peersPluginsPane.getPanel(), _("Peers"));//peersPluginsPane.getScrollPane());
	...
	*/
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_ORGS, icon_org, MainFrame.JunkPanelOrgs, _("Organizations")); // tab_organization no longer used
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_MOTS, icon_mot, MainFrame.JunkPanelMots, _("Motions"));
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_JUSTS, icon_bal, MainFrame.JunkPanelJusts, _("Justifications"));
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_JBC, icon_jus, MainFrame.JunkPanelJBC, _("Debate"));
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_NEWS, icon_news, MainFrame.JunkPanelNews, _("News"));
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_MAN, icon_wireless, MainFrame.JunkPanelMAN, _("Mobile Adhoc Networks"));
	    	...
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_CONN, icon_wireless, MainFrame.JunkPanelCONN, _("Connections"));
	...
	*/
	    	...
	    	MainFrame.tabbedPane.addTab("Addr", icon_mail, MainFrame.my_peer_address.getScrollPane(), _("My Addresses"));
	    	MainFrame.status.addPeerMeStatusListener(MainFrame.my_peer_address);
	
	    	MainFrame.peer_addresses = new PeerAddresses(false);    	
	    	MainFrame.tabbedPane.addTab("IPs", icon_mail, MainFrame.peer_addresses.getScrollPane(), _("Addresses Selected Peer"));
	    	MainFrame.status.addPeerSelectedStatusListener(MainFrame.peer_addresses);
	    	
	    	MainFrame.peer_instances = new Instances();
	    	MainFrame.tabbedPane.addTab(MainFrame.TAB_CLONE, icon_identity, MainFrame.peer_instances.getPanel(), _("Instances"));
			...
			*/
			...
			if(data.D_Organization.isIDavailable(_organizationID, DD.DEBUG) != 1) {
				if(_DEBUG) System.out.println("DD:createAndShowGUI: Temporary organization!");
				//DD.tabbedPane.setSelectedComponent(MainFrame.TAB_organization);
				MainFrame.tabbedPane.setSelectedIndex(MainFrame.TAB_ORGS_);
				return;
			}
			MainFrame.status.setSelectedOrg(new D_Organization(_organizationID));
			...
			MainFrame.status.setMeConstituent(new D_Constituent(_constID));
		...
		static public void main(String args[]) throws util.P2PDDSQLException {
			...
			if(GetOpt.optind < args.length) {
				...
				for (int i = GetOpt.optind; i < args.length; i ++) {
					potentialDatabases.add(args[args.length-1]);
				}
			}
	...
	//		}
			...
			if (DD.DEBUG) System.err.println(_("DD: main: Got DB from command line: ")+Application.db);
			if (Application.db == null) {
				...
				int returnVal = chooser.showDialog(MainFrame.frame,_("Specify Database"));
			...
			if (DD.DEBUG) System.err.println(_("DD: main: Got DB fin")+Application.db);
			...
			if (db_to_import != null) {
				...
				int q=0;
				if (!imported) {
					q = Application_GUI.ask(_("Do you want to attempt import on the next startups?"),
					...
					dbDes.close();
					if(!r){
						Application_GUI.warning(_("A failure happened while trying to migrate your mirrors and testers!\nInstall new mirrors."), _("Mirrors Failure"));
					}
				}
				if (imported || (q != 0)) {
				...
				Application_GUI.warning(_("Result trying to import data from old database:")+"\n"+db_to_import+"\n result="+(imported?_("Success"):_("Failure")), _("Importing database!"));
			}
