...
class InstancesModel extends AbstractTableModel implements TableModel, PeerListener {
	...
	public void createInstance(int row) {
		if (currentPeer == null) {
			Application_GUI.warning(_("You need to first select a peer!"), _("Creating Instance"));
			return;
		}
		String instance = Application_GUI.input(_("Provide an instance name (empty for current time)\nfor peer:")+" "+currentPeer.getName(),  _("New Instance"), JOptionPane.QUESTION_MESSAGE);
...
//        	model.update(null, null);
        	...
        	int col = Application_GUI.ask(_("Add"), _("Columns"), Arrays.copyOf(InstancesModel.columnNames, InstancesModel.columnNames.length, new Object[]{}.getClass()), null, null);
