...
class KeysCustomAction extends DebateDecideAction {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if(command == C_REFRESH) {
        	...
        	int col = Application_GUI.ask(_("Add"), _("Columns"), Arrays.copyOf(KeysModel.columnNames, KeysModel.columnNames.length, new Object[]{}.getClass()), null, null);
...
class KeysModel  extends AbstractTableModel implements TableModel, DBListener {
	...
	public void createPeer(int row) {
		D_Peer peer =
				D_Peer.getPeerByGID(Util.getString(_instancedata.get(row).get(table.key.COL_PK)), true, true);
		if(peer._peer_ID>0){
			int v = Application_GUI.ask(_("Peer already exists, do you want to change its name?"), _("Existing Peer!"), JOptionPane.OK_CANCEL_OPTION);
			if(v!=0)
				return;
		}
		String name  = Application_GUI.input(_("What name do you want for the peer"), _("Creating peer"),
	...
	public void delete(int row) {
		...
		int v = Application_GUI.ask(_("Are you sure that you want to delete instance: "+ID), _("Delete Key"), JOptionPane.OK_CANCEL_OPTION);
		if(v != 0) return;
		try {
