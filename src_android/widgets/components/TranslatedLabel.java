...
public class TranslatedLabel extends JComboBox implements ActionListener, MouseListener /*, DocumentListener, FocusListener*/, DBListener {
	...
	public TranslatedLabel(String _text) {
		block_tip = _("Double-click to edit, or use handle on the right to change!");
	...
	void init_widget(String _text, Language lang){
    	...
    	if(Application.db != null) {
    		Application.db.addListener(this, new ArrayList<String>(Arrays.asList(table.translation.TNAME)),
    				DBSelector.getHashTable(table.translation.TNAME, table.translation.value, _text));
    	}
	}
	...
	*/
	...
	void setTranslation(){
		...
		if ((DDTranslation.constituentID<=0) || (DDTranslation.organizationID<=0))
			Application_GUI.warning(_("Cannot save translation since organizationID and constituentID are not set:")+
					" ("+DDTranslation.organizationID+","+DDTranslation.constituentID+")",
					_("Translations not synchronizable"));
		try{
			ArrayList<ArrayList<Object>> sel =
				DDTranslation.db.select("select ROWID from "+table.translation.TNAME+" where "+table.translation.submitter_ID+"==? " +
					" and "+table.translation.value+" = ? and "+table.translation.translation_lang+" = ? and "+table.translation.translation_flavor+" = ?;",
					new String[]{DDTranslation.constituentID+"",
					getOriginalText(),
					DDTranslation.authorship_lang.lang,
					DDTranslation.authorship_lang.flavor}, DEBUG);
			for(int j = 0; j < sel.size(); j++) {
				DDTranslation.db.deleteNoSync(table.translation.TNAME, new String[]{"ROWID"},
...
//					DDTranslation.organizationID+""+DDTranslation.constituentID+"");
			...
			tr.global_organization_ID = D_Organization.getGlobalOrgID(Util.getStringID(DDTranslation.organizationID));
			tr.global_constituent_ID = D_Constituent.getConstituentGlobalID(Util.getStringID(DDTranslation.constituentID));
			...
			long rowID = DDTranslation.db.insert(table.translation.TNAME,
					new String[]{table.translation.global_translation_ID, table.translation.value,table.translation.value_lang,
