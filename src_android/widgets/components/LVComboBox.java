...
public class LVComboBox extends JComboBox {
	 ...
	 */
	String buildVal() {
		return buildVal(table.field_extra.SEP_list_of_values);
		/*
		int k=getItemCount();
		if (k==0) return null;
	...
	public Component setValue(Object value) {
		return setValue(value, table.field_extra.SEP_list_of_values);
	}
	/**
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if("comboBoxChanged".equals(ev.getActionCommand())) {
			...
			if((!initing&&DD.DELETE_COMBOBOX_WITHOUT_CTRL)||((ev.getModifiers()&ActionEvent.CTRL_MASK)!=0)) {
				//Object item = this.getSelectedItem();
				//if(item!=null)
				if(0==Application_GUI.ask(_("Are you sure to delete:")+"\n\""+item+"\"?", _("Delete"),JOptionPane.OK_CANCEL_OPTION)){
