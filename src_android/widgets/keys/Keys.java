...
class KeysModel  extends AbstractTableModel implements TableModel, DBListener {
	...
	public void createPeer(int row) {
		D_Peer peer =
				D_Peer.getPeerByGID(Util.getString(_keydata.get(row).get(table.key.COL_PK)), true, true);
		if (peer._peer_ID > 0) {
	...
	public void delete(int row) {
		...
		int v = Application_GUI.ask(_("Are you sure that you want to delete key: "+ID), _("Delete Key"), JOptionPane.OK_CANCEL_OPTION);
		if(v != 0) return;
		try {
