...
class  PeerAddressesModel  extends AbstractTableModel implements TableModel, DBListener {
	...
	String columnNames[]={_("Instance"),_("Type"),_("Domain"),_("UDP"),_("TCP"),_("Certified"),_("Priority"),_("Transient"),_("Loopback")};
	protected static String[] columnToolTips = {
		_("A name for the instance"),
		_("Type of the address"),
		_("IP of the target"),
		_("UDP port"),
		_("TCP port"),
		_("Is this address certified?"),
		_("The Priority Order of the Address"),
		_("Is this address stored in the databased or just detected (transient)"),
		_("Is this a Loopback address")
	};
...
//	}
	...
	private Address getAddress_Dirs(int row) {
		...
		return Identity.listing_directories_addr.get(row);
	}
...
//			System.out.println("PeerAddresses:getValueAt_Stored: peer="+peer);
	...
	private boolean store_Dirs(int row, Boolean value) {
		...
		*/
		...
		//int ok = Application.ask(_("Do you want to add this globally or to instance:")+" \""+Identity.getMyPeerInstance()+"\"\n"+crt.toLongString(), _("Global?"),JOptionPane.YES_NO_CANCEL_OPTION);
		Object[] options = new Object[]{_("Global"), _("Instance"), _("Cancel")};
		int ok = Application_GUI.ask(_("Global?"), _("Do you want to add this globally or to instance:")+
		...
		try {
			...
			if (ok != 0) {
				...
				if (dpi == null) {
					peer.putPeerInstance(peer.getInstance(), dpi = new D_PeerInstance());
					dpi.peer_instance = peer.getInstance();
					dpi.setLID(peer.get_ID(), peer.get_ID_long());
				}
				dpi.dirty = true;
				...
				crt.priority = D_Peer.getMaxCertifiedAddressPriority(dpi.addresses)+1;
				if (DEBUG) System.out.println("PeerAddress: store_Socks: store for instanceID: "+dpi);
				// if (instID <= 0) instID = D_Peer.getNullInstanceID();
			} else {
				crt.priority = D_Peer.getMaxCertifiedAddressPriority(peer.shared_addresses)+1;
				if (DEBUG) System.out.println("PeerAddress: store_Socks: store for global peer");
			}
		
			peer.addAddress(crt, _arrival_date, arrival_date, true, dpi);
...
public class PeerAddresses extends JTable implements MouseListener, PeerListener{
	...
	private void initPopupItems() {
    	...
    	//
    	certify = new JMenuItem(new PeerAddrRowAction(this, _("Certify!"), addicon,_("Store/Certify."),
    			_("Store/Certify!"),KeyEvent.VK_S, PeerAddresses.COMMAND_MENU_STORE_CERTIFY));
    	//
