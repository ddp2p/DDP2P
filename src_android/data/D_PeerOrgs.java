...
public class D_PeerOrgs extends ASNObj {
	...
	public D_PeerOrgs(long orgID) {
		...
		D_Organization org = D_Organization.getOrganizationByLID(orgID, false, false);
		this.org_name = org.name;
		this.global_organization_ID = org.getGID();
		this.global_organization_IDhash = org.getGIDH();
		served = true;
		dirty = true;
	}
		...
		if (global_organization_ID != null) {
			...
			if (peer_org_ID > 0) {
				Application.db.update(table.peer_org.TNAME,
						...
						new String[]{p.peer_ID, Util.getStringID(organization_ID), "1", last_sync_date, Util.getStringID(peer_org_ID)},
						DEBUG);
			}else{
				peer_org_ID = Application.db.insert(table.peer_org.TNAME,
					new String[]{table.peer_org.peer_ID, table.peer_org.organization_ID, table.peer_org.served, table.peer_org.last_sync_date},
					new String[]{p.peer_ID, Util.getStringID(organization_ID), "1", last_sync_date}, DEBUG);
			}
		} else {
