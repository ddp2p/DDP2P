...
//	     	                              releaseQoT[0].subQualities[1]="DoS"; 
	...
	static public ArrayList<String> getUpdateURLs() throws P2PDDSQLException{
    	...
    	urls=Application.db.select("SELECT "+table.mirror.url +
    			...
    			new String[]{"1"}, DEBUG);
    	if(urls.size()==0){
    		if(DEBUG) System.err.println(_("No URLs found in table: ")+table.mirror.TNAME);
    		return result;
    	}
	...
	static public D_MirrorInfo getUpdateInfo(String url) throws P2PDDSQLException{
    	...
    	if(updateInfo.size()==0){
    		if(DEBUG) System.err.println(_("No updateInfo record match given url found in table: ")+table.mirror.TNAME);
    		return result;
    	}
			...
			for(int i=0; i<testers; i++) {
				...
				for(int k=0; k<expect.length; k++) {
					...
					if((_test<0) || (_test>=vi.testers_data[i].tester_QoT.length)){
						Application_GUI.warning(_("Updates test not found in new releases: ")+k+":"+expect[k].test_ID, _("Your Updates Tests Specifications"));
						return false;
					}
