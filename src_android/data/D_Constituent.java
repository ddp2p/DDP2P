	...
	public long storeVerified(boolean sync, PreparedMessage pm, String orgGID, String org_local_ID, String arrival_time,
		...
		if(address!=null) {
			for(int k =0 ; k<address.length; k++) {
				...
				if(!DD.ACCEPT_TEMPORARY_AND_NEW_CONSTITUENT_FIELDS) {
					Application_GUI.warning(_("Extra Field Type for constituent.")+" "+forename+", "+surname+"\n"+
		...
		synchronized(BroadcastClient.msgs_monitor) {
			if(BroadcastClient.msgs!=null) {
				...
				String global_sender_ID = HandlingMyself_Peer.getMyPeerGID();//.getAppText(DD.APP_my_global_peer_ID);
				dm.sender = new D_Peer();
