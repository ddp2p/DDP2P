...
class D_Organization extends ASNObj implements Summary {
	 ...
	 * @return
	 */
	public static D_Organization getOrganizationByLID(long orgID, boolean keep,
			boolean create) {
		try {
			return new D_Organization(orgID);
	...
	public void init(ArrayList<Object> row, boolean extra_fields, boolean extra_creator) throws P2PDDSQLException {
		...
		creator = D_Peer.getPeerByLID(creator_ID, true); // for creator one also needs _served
		if (extra_creator) {
			if (creator == null) {
		...
		if ((this.signature_initiator == null) && (this.params.creator_global_ID != null)) {
			...
			if (sk != null) {
				...
				Application_GUI.warning(_("Update org initiator signature for:")+this.name, _("Updated org Signature"));
			}
		}
	}
	...
	@Override
	public Encoder getEncoder(ArrayList<String> directory_GIDs) {
		if(ASNSyncRequest.DEBUG)System.out.println("Encoding OrgData: "+this);
		Encoder enc = new Encoder().initSequence();
		enc.addToSequence(new Encoder(version,false));
		enc.addToSequence(new Encoder(global_organization_ID,false));
		...
		if (creator != null) enc.addToSequence(creator.getEncoder(directory_GIDs).setASN1Type(DD.TAG_AC0));
		if (neighborhoods != null) enc.addToSequence(Encoder.getEncoder(neighborhoods, directory_GIDs).setASN1Type(DD.TAG_AC3));
		if (constituents != null) enc.addToSequence(Encoder.getEncoder(constituents, directory_GIDs).setASN1Type(DD.TAG_AC4));
		if (witnesses != null) enc.addToSequence(Encoder.getEncoder(witnesses, directory_GIDs).setASN1Type(DD.TAG_AC5));
		if (motions != null) enc.addToSequence(Encoder.getEncoder(motions, directory_GIDs).setASN1Type(DD.TAG_AC6));
		if (justifications != null) enc.addToSequence(Encoder.getEncoder(justifications, directory_GIDs).setASN1Type(DD.TAG_AC7));		
		if (signatures != null) enc.addToSequence(Encoder.getEncoder(signatures).setASN1Type(DD.TAG_AC8));
		if (translations != null) enc.addToSequence(Encoder.getEncoder(translations, directory_GIDs).setASN1Type(DD.TAG_AC9));
		if (news != null) enc.addToSequence(Encoder.getEncoder(news, directory_GIDs).setASN1Type(DD.TAG_AC10));
		if (requested_data != null) enc.addToSequence(Encoder.getEncoder(requested_data, directory_GIDs).setASN1Type(DD.TAG_AC11));
	...
	public D_Organization decode(Decoder decoder) throws ASN1DecoderFail {
		...
		if(dec.getTypeByte()==DD.TAG_AC0){ creator = new D_Peer().decode(dec.getFirstObject(true)); if(DEBUG)System.out.println("OrgData cr="+creator);}
	...
	public boolean fillLocals(D_Peer __peer, RequestData new_rq, boolean tempPeer, String arrival_time) throws P2PDDSQLException {
		...
		if((this.params.creator_global_ID!=null)&&(this.creator_ID == null)) {
			...
			if(tempPeer && (creator_ID == null))  {
				...
				__peer = D_Peer.storeReceived(__peer, !DD.BLOCK_NEW_ARRIVING_PEERS_CONTACTING_ME);
				if (__peer == null) return false;
				
				D_Peer _creator = D_Peer.getPeerByGID(this.params.creator_global_ID, true, true, __peer);
				if (creator != null) _creator.loadRemote(creator);
				creator_ID = _creator.get_ID();

				String creaGID_hash = D_Peer.getGIDHashFromGID(this.params.creator_global_ID);
				if (new_rq!=null) new_rq.peers.put(creaGID_hash, DD.EMPTYDATE);
	...
	*/
	 ...
	 * @throws P2PDDSQLException
	 */
	public long store(D_Peer peer, boolean _changed[], RequestData _sol_rq, RequestData _new_rq) throws P2PDDSQLException {
	...
	public static void main(String args[]){
		try {
			...
			if(args.length>2){
				...
				D_Peer.DEBUG = Util.stringInt2bool((args[2]), false);
			}
