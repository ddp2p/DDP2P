...
//			MyselfHandling.set_my_peer_ID_UDP(peer_ID, ds);
	...
	*/
	public static DirectoryAnnouncement prepareDirectoryAnnouncement() {
		...
		synchronized (UDPServer.directoryAnnouncementLock) {
			if ((UDPServer.directoryAnnouncement == null) 
				...
				da.globalID = HandlingMyself_Peer.getMyPeerGID();//Identity.current_peer_ID.globalID;
	...
	public static void announceMyselfToDirectoriesReset() {
		...
		announceMyselfToDirectories(ds);
	}
	/**
	...
	*/
	public void pingDirectories(){
		...
		byte[]msg = uep.encode();
		__broadcastObjectToDirectoriesByUDP(msg, uep, ds);
	}
	/*
		...
		try {
			...
			for(int cnt=0; cnt<10; cnt++) {
				...
				if (global_peer_ID != null) {
					peer = D_Peer.getPeerByGID(global_peer_ID, true, false, null);// new D_Peer(global_peer_ID, false, false, true);
					peer_ID = peer.get_ID(); //table.peer.getLocalPeerID(global_peer_ID);
				}
				if (peer_ID == null) {
				...
				UpdateMessages.integrateUpdate(ra, new InetSocketAddress(10000), new Object(), global_peer_ID, ra.peer_instance, peer_ID, null,rq, peer);
