...
//			this.tableNames = tableNames.toArray(new String[]{});
		...
		if(dec.getTypeByte()==DD.TAG_AC2)
			address = new D_Peer().decode(dec.getFirstObject(true));
	...
	public boolean verifySignature() {
		...
		try {
			...
			if (DD.WARN_OF_FAILING_SIGNATURE_ONRECEPTION) {
				Application_GUI.warning(_("Failed signature verification with:")+" "+this.address.component_basic_data.globalID, _("Failed Signature Verification"));
			}
		...
		if ( !address.verifySignature(pk) ) {
			...
			System.err.println("ASNSyncRequest:verifySignature: Faulty msg="+this);
			...
			if ( 0 == Application_GUI.ask(_("Should we fix wrong signature for your peer address?")+"\n"+address.toSummaryString(),
					_("Wrong Signature"), Application_GUI.YES_NO_OPTION)){
