...
public class Connections extends util.DDP2P_ServiceThread implements DBListener{
	...
	private static void init_used_peers() {
		...
		for (int k = 0; k < tmp_peersAvailable; k++) {
			...
			long ID = Util.lval(peers.get(k).get(QUERY_ID), -1);
	...
	private static void loadAddresses_to_PeerConnection(Connection_Peer p, long peer_ID) {
		...
		D_Peer peer = D_Peer.getPeerByLID(peer_ID, true);  // not yet used when saving new served orgs
	...
	static void loadInstanceAddresses(Connection_Peer p, D_Peer peer, ArrayList<Address> addresses, D_PeerInstance dpi) {
		...
		for (int a = 0; a < addresses.size(); a ++) {
			...
			if (Address.NAT.equals(ad.getPureProtocol())) continue; // not handling NATs stored in database
			if (Address.DIR.equals(ad.getPureProtocol())) {
				...
				if (pd == null) {
					...
					if (ia != null) {
						...
						if (DEBUG) System.out.println("Connection:loadAddresses: Unknown host: "+ad);
						continue;
					}
				...
				apd.add(pd);
				if (DEBUG) System.out.println("Connection:loadAddresses: Directory: "+ad);
			} else
				if (Address.SOCKET.equals(ad.getPureProtocol())) { // socket
					...
					if (ps == null) {
						...
						if ((isa_udp != null) &&
							...
							if (DEBUG) out.println("Connections:loadAddresses: UPeer "+p.getName()+" is myself!"+isa_udp);
							isa_udp = null;
						}
					...
					aps.add(ps);
					if (DEBUG) System.out.println("Connection:loadAddresses: Socket: "+ad);
				}
		}
		/**
		...
		try {
			if (_aps != null) {
				if (DEBUG) System.out.println("Connection:loadAddresses: Socket: old peer_socks #"+_aps.size());
				for (int c = _aps.size() - 1; c >= 0; c --) {
					...
					if ((tmpsock.contacted_since_start_TCP && tmpsock.contacted_since_start_UDP) && (!tmpsock.last_contact_successful_TCP && !tmpsock.last_contact_successful_UDP)) {
						if (DEBUG) System.out.println("Connection:loadAddresses: Socket: failed old peer_sock #"+tmpsock);
						continue;
					}
					/**
					 ...
					 */
					Connections_Peer_Socket crt = locatePS(_aps, tmpsock.getAddress());
					if (crt != null) {
	...
	private static void init_my_active_directories_listings() {
		...
		if ((dirs == null) || (dirs.length == 0)) {
     	...
     	if ((dirs == null) || (dirs.length == 0)) {
    		if (! DD.WARNED_NO_DIRS) {
    			Application_GUI.warning(_("Currently you have no listing_directories for connections found at Connections initialization: " +
   	...
   	for (int k = 0; k < dirs.length; k ++) {
    		try {
    			...
    			e.printStackTrace();
    			Application_GUI.warning(_("Error for "+dirs[k]+"\nError Connections: "+e.getMessage()), _("Error installing directories"));
    			continue;
    		}
    	}
	...
	static Hashtable<String,ArrayList<Address>> getDirAddress(
		...
		if (pd.supernode_addr.isa == null) {
			...
			if (ia == null) {
				ClientSync.reportDa(dir_address.ipPort(), global_peer_ID, peer_name, null, _("Null Socket"));
	...
	private static void update_supernode_address_instance(Connection_Peer _pc, Connection_Instance dpi, String peer_key,
		...
		Hashtable<String, Hashtable<String,String>> peer_contacts_for_this_peer = null;
		String inst = null;
		if (dpi != null) inst = dpi.dpi.peer_instance;
		peer_contacts_for_this_peer = _peer_contacts_for_this_peer.get(Util.getStringNonNullUnique(inst));
		if (peer_contacts_for_this_peer == null) {
			peer_contacts_for_this_peer = new Hashtable<String, Hashtable<String,String>>();
			_peer_contacts_for_this_peer.put(Util.getStringNonNullUnique(inst), peer_contacts_for_this_peer);
		}
		...
		for (int k = 0; k < crt_peer_directories.size(); k++) {
			...
			if (adr_addresses == null) {
				...
				Hashtable<String,String> value = peer_contacts_for_this_peer.get(key);
				if (value == null) {
					value = new Hashtable<String,String>();
					peer_contacts_for_this_peer.put(key, value);
				}
			...
			getSocketAddresses_for_peerContacts_widget(//tcp_sock_addresses, udp_sock_addresses,
					...
					peer_key, 
					now, 
					peer_contacts_for_this_peer, DEBUG);
			...
			if(DEBUG) System.out.println("Connections: integrateDirAddresses: got "+peer_contacts_for_this_peer);
			s_addr_ip = null;
			for (String i : adr_addresses.keySet()) {
				ArrayList<Address> adrs = adr_addresses.get(i);
				String _s_addr_ip=Util.concat(adrs.toArray(), DirectoryServer.ADDR_SEP);
 ...
 */
	...
	public static ArrayList<Address> getKnownDirectoryAddresses (
		...
		if ((Util.equalStrings_null_or_not(instance, me.getInstance())) && Util.equalStrings_null_or_not(GIDH, me.getGIDH())) {
	...
	public static void registerIncomingDirectoryAnswer (
		...
		String ipport = directory_domain+":"+directory_udp_port;
		D_Peer me = data.HandlingMyself_Peer.get_myself();
		D_Peer target = D_Peer.getPeerByGIDhash(dami.remote_GIDhash, true, false, false);
		...
		for (DirectoryAnswerInstance inst : dami.instances) { 
			...
			if ((Util.equalStrings_null_or_not(inst.instance, me.getInstance())) && Util.equalStrings_null_or_not(dami.remote_GIDhash, me.getGIDH())) {
				...
				if (DEBUG) System.out.println("Connections: registerIncomingDirectoryAnswer: not me gidh:"+dami.remote_GIDhash+" vs "+ me.getGIDH()+" = "+Util.equalStrings_null_or_not(dami.remote_GIDhash, me.getGIDH()));
			}
			...
			if (DEBUG) {
				out.println("Connections: registerIncomingDirectoryAnswer: crt key="+ peer_key);
				for ( String pc_key : D_Peer.peer_contacts.keySet()) {
					out.println("Connections: registerIncomingDirectoryAnswer: available pc_key="+ pc_key);
					...
					for (String _ktpc : _otpc.keySet()) {
						out.println("Connections: registerIncomingDirectoryAnswer: av instance="+ _ktpc);
						Hashtable<String, Hashtable<String,String>> _tpc =  _otpc.get(_ktpc);
						for ( String tpc_key : _tpc.keySet()) {
							out.println("Connections: registerIncomingDirectoryAnswer:  available tpc_key="+ tpc_key);
							Hashtable<String,String> _tpce =  _tpc.get(tpc_key);
							for ( String tpce_key : _tpce.keySet()) {
								out.println("Connections: registerIncomingDirectoryAnswer:    available tpce_key="+ tpce_key);
								out.println("Connections: registerIncomingDirectoryAnswer:    available tpce_key="+ _tpce.get(tpce_key));
							}
						}
					}
				}
			...
			Hashtable<String, Hashtable<String,String>> p_c = null;
			if (_p_c != null) _p_c.get(Util.getStringNonNullUnique(inst.instance));
			getSocketAddresses_for_peerContacts_widget(//tcp_sock_addresses, udp_sock_addresses,
					reportedAddressesUDP,
					dami.remote_GIDhash, type, s_addr_ip, peer_key, now, p_c, DEBUG);
