 ...
 * }
	...
	public ASNUDPPing decode(Decoder dec) throws ASN1DecoderFail {
		try {
			...
			if (d.isFirstObjectTagByte(DD.TAG_AP7)||(d.getTypeByte()!=0)) peer_globalID = d.getFirstObject(true).getString();
			if (d.isFirstObjectTagByte(DD.TAG_AP8)||(d.getTypeByte()!=0)) initiator_globalID = d.getFirstObject(true).getString();
			if (d.isFirstObjectTagByte(DD.TAG_AP9)) peer_instance = d.getFirstObject(true).getString();
			if (d.isFirstObjectTagByte(DD.TAG_AP10)) initiator_instance = d.getFirstObject(true).getString();
