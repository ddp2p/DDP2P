...
public class WB_Messages extends ASNObj{
	...
	public WB_Messages decode(Decoder dec) throws ASN1DecoderFail {
		...
		if (d.getTypeByte() == DD.TAG_AC1) peers = d.getFirstObject(true).getSequenceOfAL(D_Peer.getASN1Type(), new D_Peer());
		return this;
	}
	...
	public static WB_Messages getRequestedData(ASNSyncRequest asr,
		...
		for(String gid: asr.request.peers.keySet()) {
			...
			if ((sa!=null)&&sa.hasPeer(gid, asr.request.peers.get(gid))) continue;
			try{
				D_Peer p = D_Peer.getPeerBy_or_hash(gid, true, false);
				if ((p == null) || !p.readyToSend()) continue;
				if (asr.request.peers.get(gid).compareTo(p.getCreationDate()) > 0) {
					if(DEBUG) System.out.println("WB_Messages: getRequestedData: I don't have requested peer date: "+gid+", "+asr.request.peers.get(gid));
					continue;
				}
				result.peers.add(p);
			}catch(Exception e){
				if(DEBUG) System.out.println("WB_Messages: getRequestedData: I don't have requested peer: "+gid);
				//e.printStackTrace();
			}
			if(DEBUG) System.out.println("WB_Messages: getRequestedData: got peer");
		}
	...
	public static void store(ASNSyncPayload asa, D_Peer peer, WB_Messages r,
		...
		for (D_Peer p: r.peers) {
			...
			if (! p.verifySignature()) continue;
			rq = missing_sr.get("");
			if (rq == null) rq = new RequestData();
			sol_rq = new RequestData();
			new_rq = new RequestData();
			p.fillLocals(sol_rq, new_rq);
			D_Peer old = D_Peer.getPeerByGID(p.getGID(), true, true);
			old.loadRemote(p);
			rq.update(sol_rq, new_rq);
			missing_sr.put("", rq);			
			
			obtained = obtained_sr.get("");
			if(obtained==null) obtained = new RequestData();
			obtained.peers.put(p.getGID(), DD.EMPTYDATE);
			obtained.peers.put(p.getGIDH(), DD.EMPTYDATE);
			obtained_sr.put("", obtained);
		}
		...
		for(D_Organization org: r.orgs) { // should we store new orgs like that?
					...
					)
			{
				...
				if(DD.AUTOMATE_PRIVATE_ORG_SHARING == 0) {
					int pos = Application_GUI.ask(_("In this session, do you want to share data of private orgs\n to all those sending it to you?"),
							_("Share private orgs with your providers"), Application_GUI.YES_NO_OPTION);
		...
		for(D_Neighborhood n: r.neig) {
			...
			rq = missing_sr.get(n.global_organization_ID);
			if(rq==null) rq = new RequestData();
			sol_rq = new RequestData();
			new_rq = new RequestData();
			...
			rq.update(sol_rq, new_rq);
			missing_sr.put(n.global_organization_ID, rq);			
		...
		for(D_Motion m: r.moti) {
			...
			rq = missing_sr.get(m.global_organization_ID);
			if(rq==null) rq = new RequestData();
			sol_rq = new RequestData();
			new_rq = new RequestData();
			...
			rq.update(sol_rq, new_rq);
			missing_sr.put(m.global_organization_ID, rq);			
		...
		for(D_Justification j: r.just) {
			...
			rq = missing_sr.get(j.global_organization_ID);
			if(rq==null) rq = new RequestData();
			sol_rq = new RequestData();
			new_rq = new RequestData();
			...
			rq.update(sol_rq, new_rq);
			missing_sr.put(j.global_organization_ID, rq);			
		...
		for(D_Translations t: r.tran) {
			...
			rq = missing_sr.get(t.global_organization_ID);
			if(rq==null) rq = new RequestData();
			sol_rq = new RequestData();
			new_rq = new RequestData();
			...
			rq.update(sol_rq, new_rq);
			missing_sr.put(t.global_organization_ID, rq);			
