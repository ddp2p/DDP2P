...
public class OrgHandling {
	...
	public	static boolean updateOrg(ASNSyncPayload asa, D_Organization od, String[] _orgID, String arrival_time,
		...
		*/
		...
		if (od.creator != null) {
			...
			D_Peer.storeReceived(od.creator, true);
				...
				)
		{
					...
					)
			{
				...
				if (DD.AUTOMATE_PRIVATE_ORG_SHARING == 0) {
					int r = Application_GUI.ask(_("In this session, do you want to share data of private orgs\n to all those sending it to you?"),
	...
	static D_Organization _signableOrgData(String ofi_orgID, String local_id, ArrayList<Object> row) throws P2PDDSQLException {
		...
		D_Peer creator = D_Peer.getPeerByLID(creator_ID, true); // for creator one alse needs _served
		if(creator == null) {
			if(DEBUG)Util.printCallPath("No creator:"+creator_ID);
			Application_GUI.warning(Util._("Missing organization creator, or may have incomplete data. You should create a new one!"), Util._("Missing organization creator."));
			return null;
		}
...
//				}
				...
				if(K!=null) {
					...
					if(DD.STREAM_SEND_ALL_ORG_CREATOR && (crt_org!=null)) {
						try{crt_org.creator = D_Peer.getPeerByGID(crt_org.params.creator_global_ID, true, false);}
					...
					if(!DD.STREAM_SEND_ALL_FUTURE_ORG || all.arrival_date.before(Util.getCalendar(last_sync_date))){
						...
						if(DD.STREAM_SEND_ALL_ORG_CREATOR) {
							try{all.creator = D_Peer.getPeerByGID(all.params.creator_global_ID, true, false);}
							catch(Exception e){e.printStackTrace();}
						}
					}
	...
	static boolean serving_org_to_peer(String org_id, D_Peer address) {
		...
		if((address.component_basic_data.emails!=null)&&(address.getEmailVerified())){
			try {
				dorg = new D_Organization(Util.lval(org_id, -1));
			} catch (Exception e) {
				e.printStackTrace();
