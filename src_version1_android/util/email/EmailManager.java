...
class EmailManager {
	...
	public static void main(String args[]) {
		try {
			...
			iv.verifier = D_Peer.getPeerByLID(1, true);
	...
	public static void sendEmail(DD_EmailableAttachment iv) {
		...
		try{
			if(host==null) {
				host = Application_GUI.input(_("Enter the SMTP Host"), _("Email host"),
		...
		try{
			if(user==null) {
				user = Application_GUI.input(_("Email username:"+" "+host), _("Email username"),
						Application_GUI.QUESTION_MESSAGE);
		...
		try{
			if(password==null) {
				password = Application_GUI.input(_("Password for:"+" "+user+" at "+host), _("Email password"),
						Application_GUI.QUESTION_MESSAGE);
		...
		if(to_email == null){
			Application_GUI.warning(_("Abandon as I do not know where to send the message!"), _("Verification abandoned"));
			return;
		}
