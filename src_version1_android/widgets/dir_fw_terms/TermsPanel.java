...
//    	}
    	JLabel instanceL = new JLabel(_("Select an Instance:  "));
	    ...
	    JLabel peerTopicL = new JLabel(_("Directory Tocken (Topic): "));
    	...
    	if(source == (Object)dirList) {
    		...
    		if(source == cascadeBtm){
    			...
    			if(linkToGlobal) {
    	    		...
    	    		int setThisToGeneral = Application_GUI.ask(_("Are you sure?"),
...
//				new String[]{""+peerID2}, DEBUG);
    ...
    public void queryInstance(){
		...
		if(instanceList.getItemCount()>0) {
			...
			default_for_all_Instances = _("Default for all instance");
