...
public class ListViewCanvas extends Canvas implements CommonSettings
{
	...
	protected void IgnoreUser(boolean IsIgnore, String IgnoreUserName)
	{
		...
		if (m_listIndex >= 0)
		{
			...
			if(IsIgnore)
			{
				...
				chatPeer.messagecanvas.AddMessageToMessageObject(IgnoreUserName + " has been ignored!",MESSAGE_TYPE_LEAVE);	
			}
			else
			{
				chatPeer.tappanel.cmdIgnore.setLabel("Ignore User");
				chatPeer.messagecanvas.AddMessageToMessageObject(IgnoreUserName + " has been romoved from ignored list!",MESSAGE_TYPE_JOIN);	
			}
		}	
	}
	...
	protected void IgnoreUser(boolean IsIgnore)
	{
		...
		{
			chatPeer.messagecanvas.AddMessageToMessageObject("Invalid User Selection!",MESSAGE_TYPE_ADMIN);
			return;
		}
		if (SelectedUser.equals(chatPeer.UserName))
		{
			chatPeer.messagecanvas.AddMessageToMessageObject("You can not ignored yourself!",MESSAGE_TYPE_ADMIN);	
			return;
		}
	...
	protected void SendDirectMessage()
	{
		...
		if (SelectedUser.equals(chatPeer.UserName))
		{
			chatPeer.messagecanvas.AddMessageToMessageObject("You can not chat with yourself!",MESSAGE_TYPE_ADMIN);	
			return;
		}	
	...
	private void CreatePrivateWindow()
	{
		...
		if(!(IsIgnoredUser(SelectedUser)))	
		{
			...
			{	
				...
				{
					chatPeer.messagecanvas.AddMessageToMessageObject("You are Exceeding private window limit! So you may lose some message from your friends!",MESSAGE_TYPE_ADMIN);	
				}
				else							
				{
