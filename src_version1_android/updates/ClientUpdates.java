...
public class ClientUpdates extends util.DDP2P_ServiceThread {
	...
	public void ___run() {
		...
		for(;;) {
			...
			if(!has_trusted_testers) {
				Application_GUI.warning(Util._("No trusted updates provider key. Please install an update/tester key."), Util._("No trusted updates provider key!"));
	...
	private String selectNewest(
		...
		def_option = newest;
		c = Application_GUI.ask(_("Several new version?"), _("Multiple upgrades"),
				version_ids.toArray(),
				def_option, null);
		switch(c) {
		case Application_GUI.CLOSED_OPTION:
...
//						}
	...
	public VersionInfo getNewerVersionInfos(String url_str, String ver) {
		...
		if(!a.version.equals(ver))
			{
				Application_GUI.warning(Util._("No update available from url: "+url_str), Util._("No update available"));
				return null;
			}
		// a is null if 1) invalid Mirror Signature 2) receive fault(s) from Mirror server
		if(a==null){
	...
	public static boolean downloadNewer(VersionInfo a) throws IOException {
		...
		if(!newDirFile.exists()){
			...
			if(!newDirFile.isDirectory()){
				Application_GUI.warning(_("Cannot install updates:"+"\n"+newDirName), _("Path busy for updates"));
		...
		try{
			...
			Application_GUI.warning(_("A new version is dowloaded.")+"\n" +
		...
		if(update_script.exists()&&!update_script.setExecutable(true, false)){
			Application_GUI.warning(_("Cannot set executable permission for downloaded:")+" \""+script_absolute[0]+"\".",
		...
		if(update_script.exists()&&update_script.canExecute()) {
			...
			case DD.MAC:
				...
				Application_GUI.warning(_("Let us try LINUX/MAC installers for the new version dowloaded.")+"\n" +
				...
				if(_update_script.exists()&&_update_script.canExecute()) {
					...
					Application_GUI.warning(_("No executable script found for updates")+" \""+_script_absolute[0]+"\".\n",
			...
			case DD.WINDOWS:
				...
				_update_script = new File(_script_absolute[0]);
				Application_GUI.warning(_("Let us try WINDOWS installers for the new version dowloaded.")+"\n" +
						_("Will now attempt to execute script:")+" \""+_update_script+"\"\n"+
						_("A separate confirmation will be requested before running it, if available."),
						_("New Version Downloaded"));
				if(!_update_script.setExecutable(true, false))
		...
		if(prepareLatest(install_root_path_with_sep, a.version)) {
			Application_GUI.warning(_("A new version is dowloaded. Restart your application to use it. VERSION:")+" "+a.version,
					_("New Version Downloaded"));
			return true;
		}else{
			Application_GUI.warning(
	...
	private static boolean prepareLatest(String parent_dir_with_sep, String version) {
		...
		try{
			...
			if(old_previous.exists()&&!old_previous.delete()){
				...
				Application_GUI.warning(Util._("Fail to delete ")+PREVIOUS, Util._("Download stopped. No link made"));
				return false;
			}
			...
			if(!history.exists())Util.storeStringInFile(history, version);
				...
				Application_GUI.warning(Util._("File date conflict for: ")+HISTORY, Util._("New Version Downloaded, No link made"));
				return false;
			}
			...
			if(!old_latest.exists()){
				...
				Application_GUI.warning(Util._("I QUIT. Non-standard configuration. No file:")+LATEST, Util._("Download stopped. No link made"));
				return false;
			}
			...
			if(!latest.exists()){
				...
				Application_GUI.warning(Util._("Fail to delete&install ")+LATEST, Util._("Download stopped. No link made"));
				return false;
			}
			...
			Application_GUI.warning(Util._("Fail to delete&install ")+version+"\n"+e.getLocalizedMessage(), Util._("New Version Downloaded, Exception"));
			e.printStackTrace();
			return false;
		}
	...
	public static boolean fixLATEST() throws IOException, P2PDDSQLException{
		...
		if(previous!=null) {
			...
			int c = Application_GUI.ask(_("Return to")+" \""+version+"\" "+_("(YES) or to")+" \""+previous+"\" "+"(NO)", _("What to do?"),
		...
		if(!old_latest.renameTo(bck = new File(LATEST+"."+Util.random(100)))){
			if(DEBUG)System.out.println("ClientUpdates prepareLatest: cannot rename LATEST to PREVIOUS");
			Application_GUI.warning(Util._("Fail to rename ")+LATEST, Util._("Failure to fix Latest"));
			return false;
		}
		File latest = new File(LATEST);
		if(!latest.exists()){
	...
	public static boolean create_info (String fileName, String trustedName, String outputFile, String filePath) throws IOException {
		...
		if(i==null){
			Application_GUI.warning(Util._("Inappropriate input file for updates"), Util._("Bad input file!"));
		...
		for(int k=0; k<_trusted.length; k++) {
	 	...
	 	if((i.testers_data==null)||(i.testers_data.length<1)){
	 		Application_GUI.warning(_("Need 1 tester"), _("No tester space"));
	...
	public static void main(String args[]) {
		try {
			...
			{ya = false;Application_GUI.warning(_("Input file is not good:")+input, _("Signing did not work"));}
			if(!_key.exists()|| !_key.isFile())
			{ya=false;Application_GUI.warning(_("Key file is not good:")+key, _("Signing did not work"));}
			if(_output.exists())
			{ya=false;Application_GUI.warning(_("Ouput file exists:")+output, _("Signing did not work"));}
			if(!_install.exists() || !_install.isDirectory())
			{ya=false;Application_GUI.warning(_("Install director wrong:")+install, _("Signing did not work"));}
			if(!ya){
			...
			if(!result) Application_GUI.warning(_("Something bad happened signing"), _("Signing did not work"));
