...
public class Application{
	...
	public static String CURRENT_SCRIPTS_BASE_DIR() {
		...
		if (CURRENT_SCRIPTS_BASE_DIR_WARNING && (CURRENT_SCRIPTS_BASE_DIR==null)){
			...
			Application_GUI.warning(_("No path set for scripts. Will be using \"scripts\""), _("No path set!"));
		}
	...
	public static void parseWindowsPaths(String val) {
		...
		for(String p:paths){
			...
			else {Application_GUI.warning(_("Unexpected path component type: "+p.substring(0, 2))+"\n"+_("Should be:")+"\nV=...version,R=..root.,S=...scripts,P=...plugin,D=...db,L=...logs,J=...jars",
					_("Unknown path component type"));return;}
		}
		if(":".equals(version)) {
			Application_GUI.warning(_("The WINDOWS_INSTALLATION_VERSION_BASE_DIR cannot be set to default, as it is the base of default."), _("Invalid input"));
			return;
		}
		if(":".equals(root)){
	...
	public static void parseLinuxPaths(String val) {
		...
		for(String p:paths){
			...
			if(p.startsWith("R=")){ root=p.substring(2);}
				...
				Application_GUI.warning(_("Unexpected path component type: "+p)+"\n"+_("Should be:")+"\nV=...version,R=..root.,S=...scripts,P=...plugin,D=...db,L=...logs,J=...jars",
					_("Unknown path component type"));
				return;
			}
		}
		if(":".equals(version)) {
			Application_GUI.warning(_("The LINUX_INSTALLATION_VERSION_BASE_DIR cannot be set to default, as it is the base of default."), _("Invalid input"));
			return;
		}
		if(":".equals(root)){
			root = Util.getParent(version);
